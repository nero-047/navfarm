...
const evidenceDirectory = resolve(process.cwd(), '../../docs/screenshots/phase93');
...
async function openSwitcher(page: Page) {
  ...
  await expect(page.getByRole('dialog', { name: 'Context switcher' })).toBeVisible();
}

test.beforeAll(async () => mkdir(evidenceDirectory, { recursive: true }));
test.beforeEach(async ({ page }) => reset(page));

test('audited mobile hierarchical context drawer evidence', async ({ page }) => {
  ...
  await expect(drawer.getByText('Green Valley Poultry', { exact: true })).toBeVisible();
  await expect(drawer.getByText('Harvest Ridge Farms', { exact: true })).toBeVisible();
  await expect.poll(() => drawer.getByRole('button', { name: 'Company administration', exact: true }).count()).toBeGreaterThanOrEqual(2);
  await expect(drawer.getByRole('button', { name: /Poultry Operations/ })).toBeVisible();
  await expect(drawer.getByRole('button', { name: /Feed Mill/ })).toBeVisible();
  await expect(drawer.getByRole('button', { name: /Crop Production/ })).toBeVisible();
  await expect(drawer).toHaveCSS('height', '844px');
  await page.addStyleTag({ content: 'nextjs-portal, #__next-build-watcher { display: none !important; }' });
  await expect.poll(() => page.evaluate('document.fonts.status')).toBe('loaded');
  await expect(page.locator('.animate-pulse')).toHaveCount(0);
  await expect(page.getByText(/Loading|Something went wrong|Unable to load|Request failed/i)).toHaveCount(0);
  await expect.poll(() => page.evaluate('document.documentElement.scrollWidth')).toBeLessThanOrEqual(390);
  await page.screenshot({
    path: resolve(evidenceDirectory, 'hierarchical-switcher-mobile-390x844.png'),
  });
});

test('company dashboard resolves to overview with company-only navigation and accounting', async ({ page }) => {
  ...
  const nav = page.getByRole('navigation', { name: 'Company administration navigation' });
  await expect(nav.getByRole('link', { name: 'Overview' })).toBeVisible();
  await expect(nav.getByRole('link', { name: /Batches|Operations|Quality|Reports/ })).toHaveCount(0);
  ...
  await expect(page.getByRole('button', { name: /Poultry Operations/ })).toHaveCount(0);
  ...
  await expect(page.getByText(/Operations gate|Operations ready/)).toHaveCount(0);
  await capture(page, 'company-accounting-readiness');
});

test('company and workspace selections update the full context tuple atomically', async ({ page }) => {
  await signIn(page, 'multi@navfarm.demo');
  await expect(page).toHaveURL(/\/context-selection$/);
  ...
  expect([session.activeTenantId, session.activeCompanyId, session.activeWorkspaceId]).toEqual([
  ...
  const [contextResponse] = await Promise.all([
    ...
    page.getByRole('button', { name: 'Company administration' }).first().click(),
  ]);
...
test('hierarchical switcher lists only authorized contexts and preserves supported modules', async ({ page }) => {
  await signIn(page, 'multi@navfarm.demo');
  await page.getByRole('button', { name: /Green Valley Poultry/ }).click();
  await page.getByRole('button', { name: /Poultry Operations/ }).click();
  await page.goto('/green-valley-poultry/workspaces/poultry-operations/reports');
  await openSwitcher(page);
  await expect(page.getByRole('button', { name: /Crop Production/ })).toBeVisible();
  await expect(page.getByText('Aquaculture', { exact: true })).toHaveCount(0);
  await page.getByRole('button', { name: /Crop Production/ }).click();
  await expect(page).toHaveURL(/\/harvest-ridge-farms\/workspaces\/crop-production\/reports$/);
});

test('captures the audited mobile hierarchical context drawer', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await signIn(page, 'multi@navfarm.demo');
  await page.getByRole('button', { name: /Green Valley Poultry/ }).click();
  ...
  expect((await session.json()).user.email).toBe('multi@navfarm.demo');
  ...
  await expect(drawer.getByText('Green Valley Poultry', { exact: true })).toBeVisible();
  await expect(drawer.getByText('Harvest Ridge Farms', { exact: true })).toBeVisible();
  await expect(drawer.getByRole('button', { name: 'Company administration' })).toHaveCount(2);
  await expect(drawer.getByRole('button', { name: /Poultry Operations/ })).toBeVisible();
  await expect(drawer.getByRole('button', { name: /Crop Production/ })).toBeVisible();
  await expect(drawer.getByRole('button', { name: /Feed Mill/ })).toBeVisible();
  await expect(drawer.locator('.animate-pulse')).toHaveCount(0);
  await expect(drawer.getByText(/loading|error|failed/i)).toHaveCount(0);
...
test('workspace navigation follows type, modules and workspace permissions', async ({ page }) => {
  await signIn(page, 'multi@navfarm.demo');
  await page.getByRole('button', { name: /Green Valley Poultry/ }).click();
  await page.getByRole('button', { name: /Poultry Operations/ }).click();
  let nav = page.getByRole('navigation', { name: 'Workspace operations navigation' });
  ...
  await openSwitcher(page);
  await page.getByRole('button', { name: /Crop Production/ }).click();
  nav = page.getByRole('navigation', { name: 'Workspace operations navigation' });
  await expect(nav.getByRole('link', { name: 'Production cycles' })).toBeVisible();
  await expect(nav.getByRole('link', { name: 'Traceability' })).toHaveCount(0);
  await expect(page.getByRole('heading', { name: 'Executive dashboard' })).toBeVisible();
  await capture(page, 'agriculture-workspace-navigation');
});

test('explicit workspace membership and stale company-workspace pairs are rejected', async ({ page }) => {
  await signIn(page, 'tenant@navfarm.demo');
...
test('manager and viewer navigation matches mutation authorization', async ({ page }) => {
  await signIn(page, 'manager@navfarm.demo');
  await expect(page.getByRole('navigation', { name: 'Workspace operations navigation' }).getByRole('link', { name: 'Operations' })).toBeVisible();

...
export function AccountingReadinessView({ companySlug }: { companySlug: string }) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 p-4 sm:p-6 xl:p-8">
      <PageHeader eyebrow="Accounting readiness" title="Company accounting readiness" description="Accounting readiness is recalculated from company configuration resources, not browser storage or workspace operations." />
      {error ? <ErrorState message={error} onRetry={() => void load()} /> : null}
      {!status ? <LoadingState label="Calculating readiness…" /> : <>
        <section className={`rounded-xl border p-6 ${status.operationsReady ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><span><h2 className="text-xl font-black">{status.operationsReady ? 'Accounting configuration ready' : 'Accounting configuration incomplete'}</h2><p className="mt-1 text-sm">{status.operationsReady ? 'All confirmed accounting prerequisites are configured.' : `${status.blockingRequirements.length} requirements still block accounting readiness.`}</p></span><strong className="text-5xl">{status.percentage}%</strong></div></section>

...
export function isNavigationItemActive(
  pathname: string,
  item: Pick<AppNavItem, 'href' | 'activePrefixes'>,
): boolean {
...
export function activeNavigationItem(
  ...
  return items.find((item) => isNavigationItemActive(pathname, item)) ?? null;
}
...
export function navigationForScope(
  ...
  if (scope === 'platform') {
    return [
      ...
      { label: 'Tenants', href: '/admin/tenants', activePrefixes: ['/admin/tenants'], icon: Building2, permission: 'platform.manage' },
      { label: 'Plans', href: '/admin/plans', activePrefixes: ['/admin/plans'], icon: Layers, permission: 'platform.manage' },
      { label: 'Master data', href: '/admin/masters', activePrefixes: ['/admin/masters'], icon: Database, permission: 'platform.manage' },
  ...
  if (scope === 'tenant') {
    return [
      ...
      { label: 'Companies', href: '/console/companies', activePrefixes: ['/console/companies'], icon: Building2, permission: 'company.view' },
  ...
  if (scope === 'company') {
    return [
      ...
      { label: 'Setup', href: `${root}/setup`, activePrefixes: [`${root}/setup`], icon: Building2, permission: 'company.view' },
      { label: 'Workspaces', href: `${root}/workspaces`, activePrefixes: [`${root}/workspaces`], icon: Layers, permission: 'workspaces.view' },
      { label: 'Masters', href: `${root}/masters`, activePrefixes: [`${root}/masters`], icon: Database, permission: 'masters.view' },
      { label: 'Accounting', href: `${root}/accounting/readiness`, activePrefixes: [`${root}/accounting`], icon: Landmark, permission: 'finance.view' },
      ...
      { label: 'Settings', href: `${root}/settings`, activePrefixes: [`${root}/settings`], icon: Settings, permission: 'company.view' },
    ];
  }
  ...
  return [
    ...
    { label: productionLabel, href: `${workspaceRoot}/batches`, activePrefixes: [`${workspaceRoot}/batches`], icon: Boxes, workspacePermission: 'batches.view', module: 'Batches' },

...
export default function JournalPanel() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/gl-account?${params.toString()}`)
      .then((r) => setGlAccounts(unwrap<Row[]>(r) || []))
      .catch(() => undefined);
  }, []);
  ...
  const isBalanced =
    Math.abs(lineTotals.debit - lineTotals.credit) < 0.0001 &&
    lineTotals.debit > 0;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (cleanLines.length < 2)
        throw new Error('Add at least two lines (one debit, one credit).');
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs" style={S.sub}>
            Manual entries and system-generated postings from Inventory
            movements. Must balance (debits = credits) to post.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  setHeader((h) => ({ ...h, posting_date: e.target.value }))
                }
                className={inputCls}
                style={S.input}
              />
                  ...
                  >
                    <td className="px-2 py-1.5">
                      <select
                        ...
                        onChange={(e) =>
                          setLineField(idx, 'gl_account_id', e.target.value)
                        }
                        className={inputCls}
                        style={S.input}
                      >
                    ...
                    <td className="px-2 py-1.5 w-28">
                      <input
                        ...
                        onChange={(e) =>
                          setLineField(idx, 'debit_amount', e.target.value)
                        }
                        className={inputCls}
                        style={S.input}
                      />
                    ...
                    <td className="px-2 py-1.5 w-28">
                      <input
                        ...
                        onChange={(e) =>
                          setLineField(idx, 'credit_amount', e.target.value)
                        }
                        className={inputCls}
                        style={S.input}
                      />
              ...
              <tfoot>
                ...
                >
                  ...
                  <td className="px-3 py-2" style={S.primary}>
                    {lineTotals.debit.toFixed(2)}
                  </td>
                  <td className="px-3 py-2" style={S.primary}>
                    {lineTotals.credit.toFixed(2)}
                  </td>
          ...
          viewing?.status === 'DRAFT' ? (
            <>
              ...
              <button
                onClick={handlePost}
                disabled={posting}
                className="flex items-center gap-1.5 rounded-[var(--radius-sm)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
              ...
              <div>
                ...
                <p style={S.primary}>{viewing.source}</p>
              </div>
              {viewing.source_document_no && (
                <div>
                  <p
                    className="font-semibold uppercase tracking-wider"
                    style={S.muted}
                  ...
                  <p style={S.primary}>{viewing.source_document_no}</p>
                </div>
              )}
            </div>
                ...
                <tbody>
                  {(viewing.lines || []).map((l: Row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2" style={S.primary}>
                        {Number(l.debit_amount) > 0 ? l.debit_amount : '—'}
                      </td>
                      <td className="px-3 py-2" style={S.primary}>
                        {Number(l.credit_amount) > 0 ? l.credit_amount : '—'}
                      </td>
                      <td className="px-3 py-2" style={S.sub}>
                        {l.description || '—'}
                      </td>

    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <Card className="flex flex-col justify-between p-6 hover:border-(--accent)/30 transition-all duration-300 relative overflow-hidden group">
          ...
          <div>
            <div className="flex justify-between items-start">
              <span className="text-(--text-secondary) text-[10px] font-bold uppercase tracking-widest">Active Workspace Context</span>
              <div className="p-2 rounded-xl bg-(--accent-muted) border border-(--accent)/20 text-(--accent)">
          ...
          <div className="text-[11px] text-emerald-500 font-semibold flex items-center gap-1.5 mt-6 bg-emerald-500/10 border border-emerald-500/20 py-1 px-2.5 rounded-lg w-fit">
        ...
        <Card className="flex flex-col justify-between p-6 hover:border-blue-500/30 transition-all duration-300 relative overflow-hidden group">
          ...
          <div>
            <div className="flex justify-between items-start">
              <span className="text-(--text-secondary) text-[10px] font-bold uppercase tracking-widest">Team Seat Occupancy</span>
        ...
        <Card className="flex flex-col justify-between p-6 hover:border-purple-500/30 transition-all duration-300 relative overflow-hidden group">
          ...
          <div>
            <div className="flex justify-between items-start">
              <span className="text-(--text-secondary) text-[10px] font-bold uppercase tracking-widest">Active Plan Subscription</span>
      ...
      <Card className="border-(--accent)/20 bg-(--accent)/[0.03] p-6 hover:border-(--accent)/30 transition-all duration-300">
        <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-(--text-primary) text-sm">Dynamic Subscription Limit Safeguard</h4>
          ...
          <div className="flex gap-6 w-full md:w-auto border-t md:border-t-0 border-(--border) pt-4 md:pt-0">
            <div className="flex-1 md:flex-none text-left md:text-center">
              <div className="text-[9px] text-(--text-secondary) uppercase font-bold tracking-widest">Active Users</div>
              <div className="text-base font-extrabold text-(--text-primary) mt-1 font-mono">{usersCount} <span className="text-(--text-muted) text-xs">/ {tenantPlanInfo?.max_users || 5}</span></div>
            </div>
            <div className="w-px bg-(--border) h-8 hidden md:block" />
            <div className="flex-1 md:flex-none text-left md:text-center">
              <div className="text-[9px] text-(--text-secondary) uppercase font-bold tracking-widest">Provisioned Companies</div>
              <div className="text-base font-extrabold text-(--text-primary) mt-1 font-mono">{companiesCount} <span className="text-(--text-muted) text-xs">/ {tenantPlanInfo?.max_companies || 1}</span></div>
            </div>
          </div>
        </div>
      </Card>

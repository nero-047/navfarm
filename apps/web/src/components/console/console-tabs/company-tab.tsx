...
export default function CompanyTab({
  ...
  const [userPendingDeletion, setUserPendingDeletion] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (comp) {
      setProfileForm({
        ...
        primary_color_hex: comp.primary_color_hex || '#C24332',
      });
  ...
  const handleAddCompanyAdmin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(
        isTenantAdmin
          ? 'New Company Administrator registered successfully!'
          : 'New Company Operator registered successfully!',
      );
  ...
  if (isTenantAdmin && !selectedCompanyDetails && !skipDirectory) {
    return (
      <div className="flex w-full flex-col gap-4 animate-fade-in">
        ...
        {error && (
          <div className="flex items-center gap-2 rounded-[var(--radius-md)] border border-red-200 bg-red-50 px-4 py-3 text-red-700">
            <AlertCircle className="h-4 w-4 flex-shrink-0" />
            <span className="text-sm">{error}</span>
          </div>
        )}
        {success && (
          <div className="flex items-center gap-2 rounded-[var(--radius-md)] border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-700">
            <CheckCircle className="h-4 w-4 flex-shrink-0" />
            <span className="text-sm">{success}</span>
          </div>
        )}

        <div className="flex justify-between items-center bg-(--surface) border border-(--border) p-5 rounded-[var(--radius-lg)]">
          <div className="flex flex-col gap-1">
            <h3 className="font-bold text-(--text-primary) text-base">
              Corporate Directory
            </h3>
            <p className="text-xs text-(--text-secondary)">
              Monitor and configure company business nodes under this SaaS
              tenant.
            </p>
          </div>
          <Button
        ...
        <Card className="p-0 border-(--border) bg-(--surface) overflow-hidden">
          <div className="overflow-x-auto w-full">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-(--border) text-(--text-secondary) text-xs uppercase font-bold tracking-wider">
              ...
              <tbody>
                {companies.length === 0 ? (
                  <tr>
                    ...
                    >
                      No corporate profiles registered. Click above to add one.
                    </td>
                  </tr>
                ) : (
                  companies.map((comp, idx) => (
                    <tr
                      key={comp.company_id}
                      className="border-b border-(--border) hover:bg-(--surface-raised) text-xs transition-colors"
                    >
                      <td className="p-4 text-center text-(--text-secondary) font-mono">
                        {idx + 1}
                      </td>
                      <td className="p-4">
                        <span className="font-mono text-(--text-primary) bg-(--surface-raised) px-2 py-1 rounded border border-(--border) font-semibold">
                          {comp.company_code}
                        </span>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        <div className="flex flex-col text-xs text-(--text-secondary) font-mono gap-0.5">
                      ...
                      <td className="p-4 text-right">
                        <Button
                          ...
                          className="py-1.5 px-3 text-xs uppercase font-bold tracking-wider hover:scale-[1.02] cursor-pointer"
                        >
                          Manage Profile
                        </Button>
                      </td>
          ...
          <form onSubmit={handleCreateCompany} className="flex flex-col gap-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input
                ...
                onChange={(e) =>
                  setCreateForm({ ...createForm, company_code: e.target.value })
                }
                required
              />
              <Input
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="bg-(--input-bg) border border-(--input-border) rounded-[var(--radius-md)] px-4 h-12 text-sm text-(--input-text) focus:outline-none focus:border-(--input-border-focus) cursor-pointer"
                >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex gap-2 items-center">
                  <input
                    ...
                    className="w-12 h-12 rounded-[var(--radius-md)] bg-transparent border-0 cursor-pointer"
                  />
                  <Input
                    value={createForm.primary_color_hex}
                    onChange={(e) =>
    ...
    <div className="flex w-full flex-col gap-4 animate-fade-in">
      ...
      {isTenantAdmin && !skipDirectory && (
        <button
          ...
          className="flex items-center gap-2 text-xs text-(--text-secondary) hover:text-(--text-primary) cursor-pointer w-fit font-bold bg-(--surface-raised) py-2 px-4 rounded-[var(--radius-md)] border border-(--border) transition-colors"
        >
      ...
      <div className="company-settings-container grid grid-cols-1 items-start gap-4 lg:grid-cols-12">
        ...
        <div className="flex flex-col gap-4 lg:col-span-8">
          ...
          >
            <Card
              ...
              className={`nf-company-config flex flex-col gap-5 border-(--border) bg-(--surface) p-5 ${isEditing ? 'max-h-[calc(100dvh-2rem)] overflow-y-auto shadow-[0_24px_80px_rgba(0,0,0,0.24)] sm:max-h-[calc(100dvh-3rem)]' : ''}`}
            >
              <div className="flex justify-between items-center border-b border-(--border) pb-4">
                <div>
                  ...
                  {targetCompany && (
                    <span className="text-xs text-(--text-secondary) font-mono font-semibold bg-(--surface-raised) px-1.5 py-0.5 rounded border border-(--border) mt-1 block w-fit">
                      {targetCompany.company_id}
                    </span>
                  )}
                </div>
                  ...
                  <div>
                    {!isEditing ? (
                      <button
                        ...
                        className="flex h-10 items-center gap-1.5 rounded-[var(--radius-md)] border border-(--border) bg-(--surface) px-4 text-xs font-semibold text-(--accent) transition hover:bg-(--accent-muted)"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-xs text-(--text-secondary) hover:text-(--text-primary) font-bold bg-(--surface-raised) py-1.5 px-3 rounded-[var(--radius-sm)] border border-(--border) cursor-pointer transition-colors"
                      >
              ...
              {setupLoadWarning && (
                <div className="flex items-start gap-2 rounded-[var(--radius-sm)] border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs leading-5 text-amber-800">
                ...
                <div className="flex flex-col md:flex-row gap-6 items-start">
                  ...
                  <div className="flex flex-row md:flex-col gap-1 w-full md:w-52 overflow-x-auto shrink-0 pb-3 md:pb-0 border-b md:border-b-0 md:border-r border-(--border) pr-0 md:pr-4">
                    <button
                      ...
                      className={`flex items-center gap-2 text-left px-3 py-2.5 rounded-[var(--radius-md)] text-xs font-semibold transition-all cursor-pointer whitespace-nowrap ${
                        settingsTab === 'profile'
                    ...
                    >
                      <Globe className="w-4 h-4 shrink-0" /> Steps 4-6: Locale &
                      TZ
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <Calendar className="w-4 h-4 shrink-0" /> Step 7: Fiscal
                      Config
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <Layers className="w-4 h-4 shrink-0" /> Step 8: Sectors &
                      LOBs
                    </button>
                  </div>
                    ...
                    {settingsTab === 'profile' &&
                      (!isEditing ? (
                        <div className="flex flex-col gap-6">
                          {/* Logo & Header Card */}
                          <div className="p-4 rounded-[var(--radius-md)] border border-(--border) bg-(--surface-raised) flex items-center gap-4">
                            <div className="w-16 h-16 rounded-[var(--radius-md)] border border-(--border) bg-(--input-bg) flex items-center justify-center overflow-hidden shrink-0">
                              {currentLogoUrl ? (
                                <img
                                  src={
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-4">
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs font-semibold text-(--text-primary) mt-1 font-mono bg-(--surface-raised) border border-(--border) px-3 py-1.5 rounded-[var(--radius-md)] w-fit">
                                {setupDetails?.company?.company_code ||
                                  targetCompany?.company_code}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs font-semibold text-(--text-primary) mt-1 font-mono bg-(--surface-raised) border border-(--border) px-3 py-1.5 rounded-[var(--radius-md)] w-fit">
                                {setupDetails?.company?.tax_id ||
                                  targetCompany?.tax_id ||
                                  '—'}
                              </span>
                            </div>
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs font-semibold text-(--text-primary) mt-2">
                                {setupDetails?.company?.tax_regime ||
                                  'STANDARD'}
                              </span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                                Corporate Registration No (CIN)
                              </span>
                              <span className="text-xs font-semibold text-(--text-primary) mt-1.5 font-mono bg-(--surface-raised) border border-(--border) px-3 py-1.5 rounded-[var(--radius-md)] w-fit">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs font-semibold text-(--text-primary) mt-2">
                                {setupDetails?.company?.incorporation_date ||
                                  '—'}
                              </span>
                            </div>
                            <div className="flex flex-col">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                                {setupDetails?.company?.email_domain || '—'}
                              </span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                              ...
                              <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                                {setupDetails?.company?.support_email || '—'}
                              </span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                              ...
                              <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                                {setupDetails?.company?.phone_primary || '—'}
                              </span>
                            </div>
                            <div className="flex flex-col">
                              <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                <div
                                  className="w-5 h-5 rounded-[var(--radius-sm)] border border-(--border)"
                                  style={{
                                    backgroundColor:
                        ...
                        >
                          {/* Logo Upload Section */}
                          <div className="p-4 rounded-[var(--radius-md)] border border-(--border) bg-(--surface-raised) flex flex-col sm:flex-row items-center gap-4">
                            <div className="w-16 h-16 rounded-[var(--radius-md)] border border-dashed border-(--border) bg-(--input-bg) flex items-center justify-center overflow-hidden shrink-0">
                            ...
                            <div className="flex flex-col gap-1 flex-1">
                              ...
                              <label className="cursor-pointer inline-flex items-center gap-2 px-3 py-1.5 rounded-[var(--radius-sm)] bg-(--accent)/10 text-(--accent) border border-(--accent)/30 text-xs font-semibold hover:bg-(--accent)/20 transition-all w-fit">
                          ...
                          <Button
                            ...
                            className="mt-4 self-end flex items-center gap-2 cursor-pointer text-xs"
                          >
                            <Save className="w-4 h-4" />{' '}
                            {saving
                              ? 'Saving changes...'
                              : 'Save Step 1 Profile Settings'}
                          </Button>
                        </form>
                      ))}
                      ...
                      (!isEditing ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-4">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.address?.address_label ||
                                'Primary HQ'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.address?.address_type || 'HQ'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.address?.country_id || '—'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                      ...
                      (!isEditing ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-4">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.contact?.full_name || '—'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.contact?.designation || '—'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.contact?.phone_primary || '—'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.contact?.phone_secondary || '—'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.contact?.email || '—'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                              Alerts & System Emails
                            </span>
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                            ...
                            <div className="flex flex-col gap-2 justify-center pt-2">
                              <label className="flex items-center gap-2 cursor-pointer text-xs">
                                ...
                                <span className="text-(--text-secondary) font-medium">
                                  Receive system threshold alerts
                                </span>
                              </label>
                              <label className="flex items-center gap-2 cursor-pointer text-xs">
                                <input
                                ...
                                />
                                <span className="text-(--text-secondary) font-medium">
                                  Receive periodic executive reports
                                </span>
                              </label>
                            </div>
                          </div>
                      ...
                      (!isEditing ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-4">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.company?.default_timezone_id ||
                                '—'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.company?.country_id || '—'}
                            </span>
                          </div>
                        </div>
                      ) : (
                        ...
                        >
                          ...
                          >
                            <Save className="w-4 h-4" />{' '}
                            {saving ? 'Saving changes...' : 'Save Localization'}
                          </Button>
                        </form>
                      ))}
                      ...
                      (!isEditing ? (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-4">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.fiscal?.fiscal_year_format ||
                                'FY APR-MAR'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                              Fiscal Start Month & Day
                            </span>
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.fiscal?.fiscal_start_month
                                ? `Month ${setupDetails.fiscal.fiscal_start_month}`
                                : 'Month 4 (April)'}{' '}
                              (Day {setupDetails?.fiscal?.fiscal_start_day || 1}{' '}
                              to Day{' '}
                              {setupDetails?.fiscal?.fiscal_end_day || 31})
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.fiscal?.current_fiscal_year ||
                                '2026-27'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.fiscal?.period_type || 'MONTHLY'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.fiscal?.accounting_standard ||
                                'Local GAAP'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.fiscal?.inventory_valuation ||
                                'FIFO'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                              GST / Tax Filing Frequency
                            </span>
                            <span className="text-xs font-semibold text-(--text-primary) mt-2">
                              {setupDetails?.fiscal?.gst_filing_frequency ||
                                'MONTHLY'}
                            </span>
                          </div>
                          <div className="flex flex-col">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs font-semibold text-(--text-primary) mt-2 font-mono">
                              {setupDetails?.fiscal?.decimal_places ?? 2}{' '}
                              Decimal Places
                            </span>
                          </div>
                        </div>
                    ...
                    {settingsTab === 'modules' &&
                      (!isEditing ? (
                        <div className="flex flex-col gap-4">
                          <span className="text-xs text-(--text-secondary) font-bold uppercase tracking-wider">
                            Activated Sectors & LOB Verticals
                          </span>
                          {modulesForm.length === 0 ? (
                            <div className="text-xs text-(--text-secondary) bg-(--surface-raised) p-4 rounded-[var(--radius-md)] border border-(--border)">
                              No business modules enabled. Click Edit above to
                              select sectors.
                            </div>
                          ) : (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              {nobs.map((n: any) => {
                                const isNobActive = modulesForm.includes(
                                  n.nob_code,
                                );
                                ...
                                return (
                                  <div
                                    key={n.nob_id}
                                    className="p-4 rounded-[var(--radius-md)] bg-(--surface-raised) border border-(--border) flex flex-col gap-2"
                                  >
                                    <div className="flex items-center gap-2 border-b border-(--border) pb-2">
                                      <span className="font-bold text-xs text-(--text-primary)">
                                        {n.nob_name}
                                      </span>
                                      <span className="text-xs bg-(--accent)/10 text-(--accent) font-bold border border-(--accent)/20 px-1.5 py-0.5 rounded font-mono uppercase shrink-0">
                                        Active
                                      </span>
                                    </div>
                                    <div className="flex flex-col gap-1 mt-1">
                                      <span className="text-xs font-bold text-(--accent) uppercase tracking-wider">
                                        Active Operations (LOBs):
                                      </span>
                                      {activeLobs.length === 0 ? (
                                        <span className="text-xs text-(--text-secondary)">
                                          None active
                                        ...
                                        <div className="flex flex-wrap gap-1.5 mt-1">
                                          {activeLobs.map((lob) => (
                                            <span
                                              key={lob.lob_id}
                                              className="text-xs text-(--text-secondary) bg-(--surface-raised) px-2 py-0.5 rounded border border-(--border)"
                                            >
                                              {lob.lob_name}
                                            </span>
                                          ))}
                        ...
                        <div className="flex flex-col gap-4">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {nobs.map((n: any) => {
                              const isNobChecked = modulesForm.includes(
                                n.nob_code,
                              );
                              const associatedLobs = lobMap[n.nob_id] || [];
                              ...
                              return (
                                <div
                                  key={n.nob_id}
                                  onClick={() =>
                                    handleNobToggle(n.nob_code, n.nob_id)
                                  }
                          ...
                          <Button
                            ...
                            disabled={saving}
                            className="mt-6 self-end flex items-center gap-2 cursor-pointer text-xs"
                          >
                            <Save className="w-4 h-4" />{' '}
                            {saving
                              ? 'Saving Changes...'
                              : 'Save Business Verticals'}
                          </Button>
                        </div>
                      ))}
                  </div>
              ...
              <div className="text-xs text-(--text-secondary) flex flex-col gap-4">
                <div className="flex justify-between items-center py-2 border-b border-(--border)">
                  ...
                  <span className="text-(--text-primary) font-semibold font-mono bg-(--surface-raised) px-2 py-0.5 rounded border border-(--border)">
                    {currencies.find(
                      (c) => c.currency_id === targetCompany?.base_currency_id,
                    )?.currency_name || 'INR'}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-(--border)">
                  ...
                  <span className="text-(--text-primary) font-semibold">
                    {targetCompany?.default_timezone_id || 'Asia/Kolkata'}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-(--border)">
                  <span className="text-(--text-secondary) font-medium">
                ...
                <div className="flex justify-between items-center py-2">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-bold ${
                      targetCompany?.onboarding_status === 'COMPLETED'
                        ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                        : 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20'
                    }`}
                  >
        ...
        <div className="flex flex-col gap-4 lg:col-span-4">
          ...
          <Card className="nf-company-config border-(--border) bg-(--surface) p-5">
            <div className="mb-4 flex items-center justify-between gap-3 border-b border-(--border) pb-4">
              ...
              <button
                ...
                className="nf-primary-action flex h-9 items-center gap-1.5 rounded-[var(--radius-sm)] px-3 text-xs font-semibold text-white transition hover:opacity-90"
              >
            ...
            <div className="flex flex-col gap-3 max-h-60 overflow-y-auto pr-1">
              {loadingUsers ? (
                ...
                <div className="text-xs text-(--text-secondary) text-center py-6">
                  No users assigned. Register one below.
                </div>
              ) : (
                companyUsers.map((u) => (
                  <div
                    key={u.user_id}
                    className="flex items-center justify-between rounded-[var(--radius-md)] border border-(--border) bg-(--surface-raised) p-3"
                  >
                    ...
                    <button
                      ...
                      className="p-1 text-(--text-secondary) hover:text-red-400 hover:bg-red-500/10 rounded-[var(--radius-sm)] cursor-pointer transition-colors shrink-0"
          ...
          >
            <form
              onSubmit={handleAddCompanyAdmin}
            ...
            >
              ...
              <div className="mt-1 flex flex-col-reverse gap-3 border-t border-(--border) pt-5 sm:flex-row sm:justify-end">
                ...
                <Button
                  ...
                  className="flex h-11 items-center justify-center gap-2 bg-(--accent) px-5 text-xs text-white hover:bg-(--accent-hover)"
                >
            ...
            <div className="space-y-5">
              ...
              <div className="flex flex-col-reverse gap-3 border-t border-(--border) pt-4 sm:flex-row sm:justify-end">
                <button
                  ...
                  className="min-h-10 rounded-[var(--radius-sm)] border border-(--border) bg-(--surface) px-4 text-sm font-semibold text-(--text-secondary) hover:bg-(--surface-raised)"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={() =>
                    userPendingDeletion && handleDeleteUser(userPendingDeletion)
                  }
                  className="min-h-10 rounded-[var(--radius-sm)] bg-red-600 px-5 text-sm font-semibold text-white hover:bg-red-700"
                >
                  Deactivate
                </button>
              </div>

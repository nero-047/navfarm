...
export default function BatchPanel({
  ...
  useEffect(() => {
    ...
    params.set('limit', '200');
    api
      .get(`/scheduler?${params.toString()}`)
      .then((r) => setSchedulers(unwrap<Row[]>(r) || []))
      .catch(() => setSchedulers([]));
  }, [header.lob_id]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!header.opening_quantity || !header.uom)
        throw new Error('Opening quantity and UOM are required.');
      ...
      if (cleanLines.length === 0)
        throw new Error('Add at least one input line.');
  ...
  const handleAddTransaction = async () => {
    ...
    try {
      ...
      if (['CONSUMPTION', 'OUTPUT'].includes(txForm.transaction_type)) {
        if (!txForm.item_id || !txForm.quantity || !txForm.uom)
          throw new Error(
            'Item, quantity and UOM are required for this transaction type.',
          );
        ...
        if (!txForm.quantity)
          throw new Error('Quantity is required for mortality.');
        payload.quantity = Number(txForm.quantity);
      } else if (txForm.transaction_type === 'OVERHEAD') {
        if (!txForm.quantity || !txForm.rate)
          throw new Error('Quantity and rate are required for overhead.');
        payload.quantity = Number(txForm.quantity);
  ...
  const handleClose = async () => {
    ...
    try {
      ...
      if (cleanLines.length === 0)
        throw new Error('Add at least one output line.');
    ...
    try {
      ...
      if (bioActionOpen === 'mature') {
        if (!bioForm.residual_value_per_unit)
          throw new Error('Residual value per unit is required.');
        ...
        if (!bioForm.fair_value_per_unit)
          throw new Error('Fair value per unit is required.');
        ...
        if (bioForm.disposal_type === 'HARVEST') {
          if (
            ...
            throw new Error(
              'Output item, UOM, quantity and warehouse are required for a harvest disposal.',
            );
          }
          ...
          if (!bioForm.sale_proceeds)
            throw new Error('Sale proceeds are required for a sold disposal.');
  ...
  const handleSaveQc = async () => {
    ...
    try {
      if (!qcForm.total_qty_received)
        throw new Error('Total quantity received is required.');
      ...
      if (results.length === 0)
        throw new Error('Record at least one parameter result.');
  ...
  const handleGeneratePack = async () => {
    ...
    try {
      if (!packForm.net_weight || !packForm.pack_uom)
        throw new Error('Net weight and UOM are required.');
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        {showHeader && (
          <div>
            ...
            <p className="mt-0.5 text-xs" style={S.sub}>
              Batch lifecycle: input placement, daily
              consumption/mortality/output, and closing to finished inventory.
            </p>
          </div>
        )}
      ...
      >
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead>
              <tr
                className="border-b bg-(--surface-raised) text-[13px] font-medium"
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <span
                        ...
                        style={STATUS_STYLE[row.status] || STATUS_STYLE.DRAFT}
                      >
                        {row.status}
                      </span>
                    </td>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              <label
                ...
                style={S.sub}
              >
                Scheduler (KPI monitoring)
              </label>
              <select
                value={header.scheduler_id}
                onChange={(e) =>
                  setHeader((h) => ({ ...h, scheduler_id: e.target.value }))
                }
                className={inputCls}
                style={S.input}
              ...
              >
                <option value="">
                  {header.lob_id ? 'None' : 'Select Line of Business first…'}
                </option>
                {schedulers.map((s) => (
                  <option key={s.scheduler_id} value={s.scheduler_id}>
                    {s.scheduler_code} — {s.scheduler_name}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  setHeader((h) => ({ ...h, opening_quantity: e.target.value }))
                }
                placeholder="5000"
                className={inputCls}
                style={S.input}
          ...
          <p className="text-xs" style={S.muted}>
            Rate is an estimate only — actual cost is drawn from inventory via
            FIFO when the batch is activated.
          </p>
          ...
          {header.costing_method === 'STANDARD' && (
            <>
              <div className="pt-2">
                ...
                <p className="mt-0.5 text-xs" style={S.muted}>
                  Optional — set these to enable Price/Usage/Output/Overhead
                  variance calculation when the batch closes.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    placeholder={
                      header.breed_id
                        ? 'Auto from breed mortality'
                        : 'Defaults to opening qty'
                    }
                    className={inputCls}
                    style={S.input}
                  />
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
              <div>
                ...
                <span
                  ...
                  style={STATUS_STYLE[viewing.status] || STATUS_STYLE.DRAFT}
                >
                  {viewing.status}
                </span>
              </div>
              <div>
                ...
                <p style={S.primary}>{viewing.costing_method}</p>
              </div>
              <div>
                <p
                  className="font-semibold uppercase tracking-wider"
                ...
                <p style={S.primary}>
                  {viewing.opening_quantity} {viewing.uom}
                </p>
              </div>
              {viewing.total_cost && (
                <div>
                  ...
                  <p style={S.primary}>
                    {viewing.total_cost} / {viewing.unit_cost}
                  </p>
                </div>
              )}
              {viewing.scheduler && (
                <div>
                  ...
                  <p style={S.primary}>
                    {viewing.scheduler.scheduler_code}
                    {(viewing.alerts || []).length > 0
                      ? ` — ${viewing.alerts.length} alert(s)`
                      : ''}
                  </p>
                </div>
              )}
            ...
            {viewing.status === 'DRAFT' && (
              <button
                ...
                disabled={acting}
                className="flex items-center justify-center gap-1.5 rounded-[var(--radius-sm)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50 self-start"
                style={{ backgroundColor: 'var(--accent)' }}
              >
                <PlayCircle className="h-4 w-4" />{' '}
                {acting ? 'Activating…' : 'Activate Batch'}
              </button>
            )}
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
                      ...
                      <div>
                        <p
                          ...
                          style={S.muted}
                        >
                          Monthly Amort. Rate / Unit
                        </p>
                        <p style={S.primary}>
                          {viewing.bio_asset_state.monthly_amortization_rate ??
                            '—'}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {viewing.bio_asset_state.stage === 'PREMATURE' && (
                          <button
                            ...
                            className="rounded-[var(--radius-sm)] px-3 py-1.5 text-xs font-semibold text-white"
                            style={{ backgroundColor: 'var(--accent)' }}
                          >
                            Mature Herd
                          </button>
            ...
            {viewing.status === 'ACTIVE' && (
              <div>
                ...
                <div
                  className="grid grid-cols-2 gap-2 rounded-[var(--radius-md)] border p-3 sm:grid-cols-3"
                  style={S.surface}
                >
                  <select
                  ...
                  {['CONSUMPTION', 'MORTALITY', 'OUTPUT', 'OVERHEAD'].includes(
                    txForm.transaction_type,
                  ) && (
                    <input
                      type="number"
                  ...
                  {['CONSUMPTION', 'OUTPUT'].includes(
                    ...
                    <select
                      value={txForm.uom}
                      onChange={(e) =>
                        setTxForm((f: Row) => ({ ...f, uom: e.target.value }))
                      }
                      className={inputCls}
                      style={S.input}
                    >
                  ...
                  {['OUTPUT', 'OVERHEAD'].includes(txForm.transaction_type) && (
                    <input
                      ...
                      value={txForm.rate}
                      onChange={(e) =>
                        setTxForm((f: Row) => ({ ...f, rate: e.target.value }))
                      }
                      className={inputCls}
                      style={S.input}
                    />
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setTxForm((f: Row) => ({ ...f, remarks: e.target.value }))
                    }
                  ...
                  <button
                    ...
                    disabled={acting}
                    className="rounded-[var(--radius-sm)] px-3 py-2 text-xs font-semibold text-white disabled:opacity-50 sm:col-span-3"
                    style={{ backgroundColor: 'var(--accent)' }}
                  >
            ...
            {viewing.status === 'ACTIVE' &&
              viewing.costing_method !== 'BIO_ASSET' && (
                <button
                  onClick={openClose}
                  className="flex items-center justify-center gap-1.5 rounded-[var(--radius-sm)] px-4 py-2 text-sm font-semibold text-white self-start"
                  style={{ backgroundColor: 'var(--success)' }}
                >
                      ...
                      <tbody>
                        {(viewing.variances || []).map((v: Row) => (
                          ...
                          >
                            ...
                            <td className="px-3 py-2" style={S.primary}>
                              {Number(v.std_value).toLocaleString(undefined, {
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between pt-2">
            <p
              ...
              style={S.sub}
            >
              Output Lines — split % must sum to 100
            </p>
            <button
            ...
            <table className="w-full text-left text-xs">
              ...
              <tfoot>
                <tr
                  ...
                  style={{ borderColor: 'var(--border)' }}
                >
                  <td colSpan={2} className="px-3 py-2" style={S.sub}>
                    Split Total
                  </td>
                  <td
                    className="px-3 py-2"
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="flex flex-col gap-1.5">
                <label
                  ...
                  style={S.sub}
                >
                  Residual Value / Unit <span className="text-red-500">*</span>
                </label>
                <input
                  type="number"
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  placeholder="From breed if left blank"
                  className={inputCls}
                  style={S.input}
                />
              </div>
          ...
          {bioActionOpen === 'amortize' && (
            <div className="flex flex-col gap-1.5">
              ...
              />
              <p className="text-xs" style={S.muted}>
                One amortization run per calendar month.
              </p>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col gap-1.5">
                <label
                  ...
                  style={S.sub}
                >
                  New Fair Value / Unit <span className="text-red-500">*</span>
                </label>
                <input
                  type="number"
            ...
            <div className="flex flex-col gap-4">
              ...
              {bioForm.disposal_type === 'HARVEST' ? (
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  />
                  <p className="text-xs" style={S.muted}>
                    Gain/loss vs. the disposed animals' book value posts
                    automatically. Cash/receivable isn't recorded here.
                  </p>
                </div>
              )}
            </div>
      ...
      <Dialog
        ...
        title={
          qcLine ? `Record QC — ${itemLabel(qcLine.item_id)}` : 'Record QC'
        }
        maxWidth="lg"
        footer={
          qcSubmitted ? (
            <button
              ...
              className="rounded-[var(--radius-sm)] px-4 py-2 text-sm font-semibold text-white"
              style={{ backgroundColor: 'var(--accent)' }}
            >
              Done
            </button>
        ...
        <div className="flex flex-col gap-4">
          ...
          {qcSubmitted ? (
            <div className="flex flex-col items-center gap-3 py-6">
              ...
              <div
                className="w-full overflow-x-auto rounded-[var(--radius-md)] border"
                style={S.surface}
              >
                <table className="w-full text-left text-xs">
                  <thead>
            ...
            <>
              ...
              <div>
                ...
                {qcParameters.length === 0 ? (
                  ...
                  >
                    No QC parameters defined for this Line of Business yet.
                  </p>
                ) : (
                  <div className="flex flex-col gap-2">
                    {qcParameters.map((p) => (
                      <div
                        key={p.param_id}
                        className="grid grid-cols-3 items-center gap-2 rounded-[var(--radius-md)] border p-2.5"
                        style={S.surface}
                      >
                        <div>
                          <p
      ...
      <Dialog
        ...
        title={
          packLine
            ? `Generate Pack — ${itemLabel(packLine.item_id)}`
            : 'Generate Pack'
        }
        footer={
          generatedPack ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="flex flex-col gap-1.5">
                <label
                  ...
                  style={S.sub}
                >
                  Pack UOM <span className="text-red-500">*</span>
                </label>
                <select

...
const emptyLine = () => ({
  parameter_id: "", period_no: "1", period_from: "", period_to: "", period_label: "",
...
export default function SchedulerPanel() {
  ...
  const [header, setHeader] = useState<Row>({ lob_id: "", scheduler_code: "", scheduler_name: "", duration_value: "", duration_unit: "DAY", breed_id: "", description: "" });
  ...
  const openCreate = () => {
    setNobId("");
    setHeader({ lob_id: "", scheduler_code: "", scheduler_name: "", duration_value: "", duration_unit: "DAY", breed_id: "", description: "" });
    setLines([emptyLine()]);
    setFormError("");
    setModalOpen(true);
  };
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input value={header.scheduler_code} onChange={(e) => setHeader((h) => ({ ...h, scheduler_code: e.target.value }))} placeholder="SCH-PLT-CB-42D" className={inputCls} style={S.input} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-semibold uppercase tracking-wider" style={S.sub}>Name <span className="text-red-500">*</span></label>
              <input value={header.scheduler_name} onChange={(e) => setHeader((h) => ({ ...h, scheduler_name: e.target.value }))} placeholder="Broiler 42-Day Standard" className={inputCls} style={S.input} />
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <input type="number" value={header.duration_value} onChange={(e) => setHeader((h) => ({ ...h, duration_value: e.target.value }))} placeholder="42" className={inputCls} style={S.input} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-semibold uppercase tracking-wider" style={S.sub}>Duration Unit</label>
              <select value={header.duration_unit} onChange={(e) => setHeader((h) => ({ ...h, duration_unit: e.target.value }))} className={inputCls} style={S.input}>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b" style={{ borderColor: "var(--border)" }}>
                  ...
                  <th className="px-3 py-2 font-semibold" style={S.sub}>Day From</th>
                  <th className="px-3 py-2 font-semibold" style={S.sub}>Day To</th>
                ...
                {lines.map((line, idx) => (
                  <tr key={idx} className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                    <td className="px-2 py-1.5 min-w-[180px]">
                      <select value={line.parameter_id} onChange={(e) => setLineField(idx, "parameter_id", e.target.value)} className={inputCls} style={S.input}>
                        <option value="">Select…</option>
                        {parameters.map((p) => <option key={p.parameter_id} value={p.parameter_id}>{p.parameter_code}</option>)}
                      </select>
                    </td>
                    <td className="px-2 py-1.5 w-20"><input type="number" value={line.period_from} onChange={(e) => setLineField(idx, "period_from", e.target.value)} className={inputCls} style={S.input} /></td>
                    <td className="px-2 py-1.5 w-20"><input type="number" value={line.period_to} onChange={(e) => setLineField(idx, "period_to", e.target.value)} className={inputCls} style={S.input} /></td>
                    <td className="px-2 py-1.5 w-32"><input value={line.period_label} onChange={(e) => setLineField(idx, "period_label", e.target.value)} placeholder="Week 1" className={inputCls} style={S.input} /></td>
                    <td className="px-2 py-1.5 w-28"><input type="number" value={line.expected_qty_override} onChange={(e) => setLineField(idx, "expected_qty_override", e.target.value)} placeholder="From param" className={inputCls} style={S.input} /></td>
                    <td className="px-2 py-1.5 w-24">
                      <select value={line.kpi_mode} onChange={(e) => setLineField(idx, "kpi_mode", e.target.value)} className={inputCls} style={S.input}>
                    ...
                    {line.kpi_mode === "VALUE" ? (
                      <>
                        <td className="px-2 py-1.5 w-20"><input type="number" value={line.kpi_min_value} onChange={(e) => setLineField(idx, "kpi_min_value", e.target.value)} className={inputCls} style={S.input} /></td>
                        <td className="px-2 py-1.5 w-20"><input type="number" value={line.kpi_max_value} onChange={(e) => setLineField(idx, "kpi_max_value", e.target.value)} className={inputCls} style={S.input} /></td>
                      </>
                    ) : (
                      <>
                        <td className="px-2 py-1.5 w-20"><input type="number" value={line.kpi_min_pct} onChange={(e) => setLineField(idx, "kpi_min_pct", e.target.value)} placeholder="90" className={inputCls} style={S.input} /></td>
                        <td className="px-2 py-1.5 w-20"><input type="number" value={line.kpi_max_pct} onChange={(e) => setLineField(idx, "kpi_max_pct", e.target.value)} placeholder="110" className={inputCls} style={S.input} /></td>
                      </>
                    )}
        ...
        {viewing && (
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-4">
              <div><p className="font-semibold uppercase tracking-wider" style={S.muted}>Duration</p><p style={S.primary}>{viewing.duration_value} {viewing.duration_unit}(S)</p></div>
              <div><p className="font-semibold uppercase tracking-wider" style={S.muted}>Status</p><p style={S.primary}>{viewing.is_locked ? "Locked" : "Editable"}</p></div>
            </div>
            <div className="overflow-x-auto rounded-xl border" style={S.surface}>
              <table className="w-full text-left text-xs">
                <thead><tr className="border-b" style={{ borderColor: "var(--border)" }}>
                  <th className="px-3 py-2 font-semibold" style={S.sub}>Parameter</th>
                  <th className="px-3 py-2 font-semibold" style={S.sub}>Days</th>
                  <th className="px-3 py-2 font-semibold" style={S.sub}>Label</th>
                  <th className="px-3 py-2 font-semibold" style={S.sub}>KPI</th>
                </tr></thead>
                <tbody>
                  {(viewing.parameter_lines || []).map((l: Row) => (
                    <tr key={l.spl_id} className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                      <td className="px-3 py-2" style={S.primary}>{parameterLabel(l.parameter_id)}</td>
                      <td className="px-3 py-2" style={S.sub}>{l.period_from}–{l.period_to}</td>
                      <td className="px-3 py-2" style={S.sub}>{l.period_label || "—"}</td>
                      <td className="px-3 py-2" style={S.primary}>
                        {l.kpi_mode === "VALUE" ? `${l.kpi_min_value ?? "-∞"} – ${l.kpi_max_value ?? "∞"}` : l.kpi_mode === "PCT" ? `${l.kpi_min_pct ?? 0}% – ${l.kpi_max_pct ?? "∞"}%` : "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>

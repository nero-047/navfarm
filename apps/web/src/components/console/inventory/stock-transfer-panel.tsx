  ...
  const handlePost = async () => {
    ...
    try {
      const res = await api.post(
        `/stock-transfer/${viewing.transfer_id}/post`,
        {},
      );
      setViewing(unwrap<Row>(res));
      load();
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs" style={S.sub}>
            Move stock between warehouses. Cost carries forward from FIFO layers
            at the source.
          </p>
        </div>
                  ...
                  >
                    ...
                    >
                      <span className="inline-flex items-center gap-1.5">
                        {warehouseLabel(row.from_warehouse_id)}{' '}
                        <ArrowRight className="h-3 w-3" style={S.muted} />{' '}
                        {warehouseLabel(row.to_warehouse_id)}
                      </span>
                    </td>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                onChange={(e) =>
                  setHeader((h) => ({ ...h, to_warehouse_id: e.target.value }))
                }
                className={inputCls}
                style={S.input}
              >
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-3">
              ...
              <div>
                ...
                <p style={S.primary}>
                  {warehouseLabel(viewing.from_warehouse_id)} →{' '}
                  {warehouseLabel(viewing.to_warehouse_id)}
                </p>
              </div>
              {viewing.posted_at && (
                <div>

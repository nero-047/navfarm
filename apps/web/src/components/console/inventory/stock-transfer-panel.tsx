...
import { Plus, Trash2, Search, AlertCircle, Loader2, Inbox, Eye, CheckCircle2, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold" style={S.primary}>Stock Transfer</h2>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b text-[10px] font-bold uppercase tracking-wider" style={{ ...S.sub, borderColor: "var(--border)" }}>
                <th className="whitespace-nowrap px-4 py-3">Transfer No.</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-4 py-10 text-center text-xs" style={S.sub}><Inbox className="mx-auto mb-2 h-6 w-6" style={S.muted} /> No stock transfers yet.</td></tr>
              ) : (
                rows.map((row) => (
                  <tr key={row.transfer_id} className="border-b text-xs transition-colors hover:bg-(--surface-raised)" style={{ borderColor: "var(--border)" }}>
                    <td className="whitespace-nowrap px-4 py-3 font-semibold" style={S.primary}>{row.transfer_no}</td>
                    <td className="whitespace-nowrap px-4 py-3" style={S.primary}>{row.posting_date}</td>
                    <td className="whitespace-nowrap px-4 py-3" style={S.primary}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-semibold uppercase tracking-wider" style={S.sub}>From Warehouse <span className="text-red-500">*</span></label>
              <select value={header.from_warehouse_id} onChange={(e) => setHeader((h) => ({ ...h, from_warehouse_id: e.target.value }))} className={inputCls} style={S.input}>
                <option value="">Select…</option>
                {warehouses.map((w) => <option key={w.warehouse_id} value={w.warehouse_id}>{w.warehouse_code} — {w.warehouse_name}</option>)}
              </select>
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-semibold uppercase tracking-wider" style={S.sub}>To Warehouse <span className="text-red-500">*</span></label>
              <select value={header.to_warehouse_id} onChange={(e) => setHeader((h) => ({ ...h, to_warehouse_id: e.target.value }))} className={inputCls} style={S.input}>
                <option value="">Select…</option>
                {warehouses.map((w) => <option key={w.warehouse_id} value={w.warehouse_id}>{w.warehouse_code} — {w.warehouse_name}</option>)}
              </select>
            </div>

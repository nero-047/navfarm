...
export default function GoodsIssuePanel() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/cost-center?${qs}`)
      .then((r) => setCostCenters(unwrap<Row[]>(r) || []))
      .catch(() => undefined);
    api
      .get(`/item?${qs}`)
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs" style={S.sub}>
            Record stock consumption. Cost is drawn automatically via FIFO from
            existing inventory layers.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                onChange={(e) =>
                  setHeader((h) => ({ ...h, cost_center_id: e.target.value }))
                }
                className={inputCls}
                style={S.input}
              >
                ...
                {costCenters.map((c) => (
                  <option key={c.cost_center_id} value={c.cost_center_id}>
                    {c.cost_center_code} — {c.cost_center_name}
                  </option>
                ))}
              </select>
            </div>

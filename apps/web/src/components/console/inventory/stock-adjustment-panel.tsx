        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input value={header.reason} onChange={(e) => setHeader((h) => ({ ...h, reason: e.target.value }))} placeholder="Physical count variance / Spoilage" className={inputCls} style={S.input} />
            </div>
            <div className="flex flex-col gap-1.5">
          ...
          <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: "var(--border)" }}>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider" style={S.primary}>Stock Adjustment Lines ({lines.length})</p>
              <p className="text-[11px]" style={S.muted}>Quantity adjustments (use positive values for surplus, negative for write-off).</p>
            </div>
            <Button onClick={addLine} type="button" size="sm" variant="outline" className="text-xs">
              <Plus className="h-3.5 w-3.5 mr-1" /> Add Adjustment Line
            </Button>
          </div>
              ...
              <table className="w-full border-collapse text-left text-xs">
                <TableHeader>
                  <tr className="border-b border-(--row-border)">
                    ...
                    <TableHead className="h-auto px-3 py-2.5 w-36 text-right">Adjustment Qty (±)</TableHead>
                    <TableHead className="h-auto px-3 py-2.5 w-28">UOM</TableHead>
                    <TableHead className="h-auto px-3 py-2.5 w-32 text-right">Unit Rate (if +)</TableHead>
                ...
                <TableBody>
                  {lines.map((line, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell className="px-2 py-1.5">
                        <input type="number" step="any" placeholder="e.g. -5 or 10" value={line.quantity} onChange={(e) => setLineField(idx, "quantity", e.target.value)} className={`${inputCls} text-xs text-right font-mono`} style={S.input} />
                      </TableCell>
                      <TableCell className="px-2 py-1.5">
                      ...
                      <TableCell className="px-2 py-1.5">
                        <input
                          ...
                          className={`${inputCls} text-xs text-right font-mono disabled:opacity-40`}
                          style={S.input}
                        />
                      </TableCell>

      ...
      const cleanLines = lines
        ...
        .map((l) => {
          ...
          if (quantity > 0 && !l.rate) {
            throw new Error(
              'Rate is required for lines with a positive (found stock) quantity.',
            );
          }
          return {
            item_id: l.item_id,
  ...
  const handlePost = async () => {
    ...
    try {
      const res = await api.post(
        `/stock-adjustment/${viewing.adjustment_id}/post`,
        {},
      );
      setViewing(unwrap<Row>(res));
      load();
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs" style={S.sub}>
            Correct stock for physical counts, damage or loss. Positive quantity
            = found stock (needs a rate); negative = missing/damaged (costed via
            FIFO).
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                value={header.reason}
                onChange={(e) =>
                  setHeader((h) => ({ ...h, reason: e.target.value }))
                }

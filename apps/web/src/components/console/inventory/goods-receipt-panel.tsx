...
export default function GoodsReceiptPanel() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/supplier?${qs}`)
      .then((r) => setSuppliers(unwrap<Row[]>(r) || []))
      .catch(() => undefined);
    api
      .get(`/item?${qs}`)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (cleanLines.length === 0)
        throw new Error('Add at least one line with item, quantity and UOM.');
  ...
  const handlePost = async () => {
    ...
    try {
      const res = await api.post(
        `/goods-receipt/${viewing.receipt_id}/post`,
        {},
      );
      setViewing(unwrap<Row>(res));
      load();
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs" style={S.sub}>
            Record incoming stock. Draft, then Post to write it to the Inventory
            Ledger.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                onChange={(e) =>
                  setHeader((h) => ({ ...h, warehouse_id: e.target.value }))
                }
                className={inputCls}
                style={S.input}
              >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                value={header.external_reference_no}
                onChange={(e) =>
                  setHeader((h) => ({
                    ...h,
                    external_reference_no: e.target.value,
                  }))
                }
                placeholder="Supplier DC/Invoice No."
                className={inputCls}
                style={S.input}
              />
            </div>
                  ...
                  >
                    <td className="px-2 py-1.5">
                      <select
                        value={line.item_id}
                        onChange={(e) =>
                          setLineField(idx, 'item_id', e.target.value)
                        }
                        className={inputCls}
                        style={S.input}
                      >
                    ...
                    <td className="px-2 py-1.5 w-28">
                      <select
                        value={line.uom}
                        onChange={(e) =>
                          setLineField(idx, 'uom', e.target.value)
                        }
                        className={inputCls}
                        style={S.input}
                      >
                    ...
                    <td className="px-2 py-1.5 w-24">
                      <input
                        ...
                        value={line.rate}
                        onChange={(e) =>
                          setLineField(idx, 'rate', e.target.value)
                        }
                        className={inputCls}
                        style={S.input}
                      />
                    ...
                    <td className="px-2 py-1.5 w-28">
                      <input
                        value={line.lot_no}
                        onChange={(e) =>
                          setLineField(idx, 'lot_no', e.target.value)
                        }
                        className={inputCls}
                        style={S.input}
                      />
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-3">
              ...
              <div>
                ...
                <p style={S.primary}>{viewing.external_reference_no || '—'}</p>
              </div>
              {viewing.posted_at && (
                <div>
                  <p

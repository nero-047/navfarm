      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: "var(--border)" }}>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider" style={S.primary}>Receipt Inbound Lines ({lines.length})</p>
              <p className="text-[11px]" style={S.muted}>Items, quantities, rates and batch lot numbers received into inventory.</p>
            </div>
            <Button onClick={addLine} type="button" size="sm" variant="outline" className="text-xs">
              <Plus className="h-3.5 w-3.5 mr-1" /> Add Receipt Line
            </Button>
          </div>
              ...
              <table className="w-full border-collapse text-left text-xs">
                <TableHeader>
                  <tr className="border-b border-(--row-border)">
                    ...
                    <TableHead className="h-auto px-3 py-2.5 w-24 text-right">Qty</TableHead>
                    <TableHead className="h-auto px-3 py-2.5 w-24">UOM</TableHead>
                    <TableHead className="h-auto px-3 py-2.5 w-28 text-right">Unit Rate</TableHead>
                    <TableHead className="h-auto px-3 py-2.5 w-28">Lot / Batch No.</TableHead>
                    <TableHead className="h-auto px-3 py-2.5 w-32">Expiry Date</TableHead>
                ...
                <TableBody>
                  {lines.map((line, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell className="px-2 py-1.5">
                        <input type="number" step="any" placeholder="0.00" value={line.rate} onChange={(e) => setLineField(idx, "rate", e.target.value)} className={`${inputCls} text-xs text-right font-mono`} style={S.input} />
                      </TableCell>
                      <TableCell className="px-2 py-1.5">
                        <input value={line.lot_no} onChange={(e) => setLineField(idx, "lot_no", e.target.value)} placeholder="Optional" className={`${inputCls} text-xs`} style={S.input} />
                      </TableCell>
                      <TableCell className="px-2 py-1.5">
                        <input type="date" value={line.expiry_date} onChange={(e) => setLineField(idx, "expiry_date", e.target.value)} className={`${inputCls} text-xs`} style={S.input} />
                      </TableCell>

  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold" style={S.primary}>Goods Receipt</h2>
              ...
              {loading ? (
                ...
                rows.map((row) => (
                  <tr key={row.receipt_id} className="border-b text-xs transition-colors hover:bg-(--surface-raised)" style={{ borderColor: "var(--border)" }}>
                    ...
                    <td className="whitespace-nowrap px-4 py-3" style={S.sub}>{row.external_reference_no || "—"}</td>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select value={header.supplier_id} onChange={(e) => setHeader((h) => ({ ...h, supplier_id: e.target.value }))} className={inputCls} style={S.input}>
                <option value="">Select…</option>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b" style={{ borderColor: "var(--border)" }}>
                  ...
                  <th className="px-3 py-2 font-semibold" style={S.sub}>Lot No.</th>
                  <th className="px-3 py-2 font-semibold" style={S.sub}>Expiry</th>
              ...
              <tbody>
                {lines.map((line, idx) => (
                  <tr key={idx} className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                    ...
                    <td className="px-2 py-1.5 w-24"><input type="number" value={line.rate} onChange={(e) => setLineField(idx, "rate", e.target.value)} className={inputCls} style={S.input} /></td>
                ...
                <tbody>
                  {(viewing.lines || []).map((l: Row) => (
                    <tr key={l.line_id} className="border-b last:border-0" style={{ borderColor: "var(--border)" }}>
                      ...
                      <td className="px-3 py-2" style={S.primary}>{l.lot_no || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

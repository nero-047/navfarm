...
interface OperationalConsoleProps {
  activeConsoleTab:
    'dashboard' | 'company' | 'users' | 'roles' | 'audit' | 'notification';
  setActiveConsoleTab: (
    tab: 'dashboard' | 'company' | 'users' | 'roles' | 'audit' | 'notification',
  ) => void;
  activeCompany: any;
...
export default function OperationalConsole({
  ...
  const hasPermission = (
    ...
    return perms.some((p: any) => {
      const matchesModule =
        p.moduleCode === 'ALL' || p.moduleCode === moduleCode;
      ...
      >
        {[
          ...
          },
          {
            ...
            perm: () => hasPermission('COMPANY', 'SETTINGS', 'can_view'),
          },
          {
            tab: 'users',
            label: 'Team Management',
            icon: <Users className="w-4 h-4" />,
            perm: () => hasPermission('RBAC', 'USER', 'can_view'),
          },
          {
            tab: 'roles',
            label: 'Role Permissions',
            icon: <ShieldAlert className="w-4 h-4" />,
            perm: () => hasPermission('RBAC', 'ROLE', 'can_view'),
          },
          {
            tab: 'audit',
            label: 'Audit Ledger',
            icon: <History className="w-4 h-4" />,
            perm: () => hasPermission('AUDIT', 'LOGS', 'can_view'),
          },
          {
            tab: 'notification',
            label: 'Notifications',
            icon: <Bell className="w-4 h-4" />,
            perm: () => hasPermission('NOTIFICATION', 'SETTINGS', 'can_view'),
          },
        ]
          ...
          .map((item) => {
            ...
            return (
              <button
                key={item.tab}
                onClick={() => setActiveConsoleTab(item.tab as any)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 10,
      ...
      >
        ...
        >
          ...
          >
            ...
            {hasPermission('COMPANY', 'SETTINGS', 'can_view') && (
              <option value="company">Company Details</option>
            )}
            {hasPermission('RBAC', 'USER', 'can_view') && (
              <option value="users">Team Management</option>
            )}
            {hasPermission('RBAC', 'ROLE', 'can_view') && (
              <option value="roles">Role Permissions</option>
            )}
            {hasPermission('AUDIT', 'LOGS', 'can_view') && (
              <option value="audit">Audit Ledger</option>
            )}
            {hasPermission('NOTIFICATION', 'SETTINGS', 'can_view') && (
              <option value="notification">Notification Engine</option>
            )}
          </select>
        </div>
        ...
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-500">
            ...
            <span className="font-mono text-xs md:text-xs text-gray-400">
              {tenantPlanInfo?.db_name || 'tenant_system'}
            </span>
          </p>
        </div>
        ...
        {activeConsoleTab === 'audit' && <AuditTab auditLogs={auditLogs} />}
        {activeConsoleTab === 'notification' && (

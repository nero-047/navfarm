...
export default function AdminAuditPage() {
  ...
  const loadLogs = async () => {
    ...
    try {
      ...
      setError(e?.message || t('failedToLoadAuditLogs'));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 xl:p-8">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          >
            {auditLogs.length} {t('platformWideEventsRecorded')}
          </p>
        </div>
        <div className="w-full sm:w-auto">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-[var(--radius-sm)] border py-2 pl-9 pr-4 text-sm sm:w-64"
              style={{
      ...
      >
        <table className="w-full text-sm">
          <thead>
            ...
            >
              {['#', t('timestamp'), t('action'), t('entity'), t('user')].map(
                (h, idx) => (
                  <th
                    key={idx}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((log, idx) => (
                ...
                >
                  ...
                  <td
                    className="px-5 py-3.5 font-mono text-xs whitespace-nowrap"
                    style={{ color: 'var(--text-secondary)' }}
                  >
                    {log.created_at

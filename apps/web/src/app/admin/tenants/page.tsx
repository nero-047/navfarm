...
export default function AdminTenantsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (Array.isArray(nobsData) && nobsData.length > 0) {
        ...
        const allLobIds = Object.values(lobMap)
          .flat()
          .map((l: any) => l.lob_id);
  ...
  const handleExpand = async (tenant: any) => {
    if (expandedId === tenant.tenant_id) {
      setExpandedId(null);
      return;
    }
  ...
  const handleUpgrade = async (e: React.FormEvent) => {
    ...
    setSuccess('');
    try {
      await api.post(`/tenant/${upgradingTenant.tenant_id}/change-plan`, {
  ...
  const handleCreateTenant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(
        `Tenant account ${createForm.tenant_name} registered successfully!`,
      );
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 xl:p-8">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm mt-0.5" style={S.sub}>
            {tenants.length} tenants registered on platform
          </p>
        </div>
        <div className="flex w-full items-center gap-3 sm:w-auto">
          <div className="relative min-w-0 flex-1 sm:flex-none">
            ...
            <input
              ...
              className="w-full rounded-[var(--radius-sm)] border py-2 pl-9 pr-4 text-sm sm:w-60"
              style={S.input}
            />
          </div>
          <button
            ...
            className="flex min-h-10 shrink-0 items-center gap-2 rounded-[var(--radius-sm)] px-4 text-sm font-semibold text-white shadow-sm"
      ...
      <Dialog
        ...
        description={
          upgradingTenant
            ? `Choose a new plan for ${upgradingTenant.tenant_name}.`
            : undefined
        }
        maxWidth="sm"
      >
        <form onSubmit={handleUpgrade} className="space-y-5">
          ...
          <select
            ...
            className="min-h-11 w-full rounded-[var(--radius-sm)] border px-3 text-sm"
            style={S.input}
          >
          ...
          >
            ...
            <button
              ...
              className="min-h-10 rounded-[var(--radius-sm)] bg-(--accent) px-5 text-sm font-semibold text-white hover:bg-(--accent-hover) disabled:opacity-50"
            >
      ...
      {/* Tenants Table */}
      <div
        className="rounded-[var(--radius-sm)] border shadow-sm overflow-hidden"
        style={S.surface}
      >
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b" style={S.raised}>
              {['#', 'Tenant Name', 'Email', 'Plan', 'Status', 'Actions'].map(
                (h) => (
                  <th
                    key={h}
                ...
                <React.Fragment key={tenant.tenant_id}>
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 font-semibold" style={S.primary}>
                      ...
                      <div className="text-xs font-mono mt-0.5" style={S.muted}>
                        {tenant.tenant_code}
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-xs" style={S.sub}>
                    ...
                    <td className="px-5 py-3.5">
                      <span
                        className={`text-xs font-semibold border px-2 py-0.5 rounded inline-flex items-center gap-1 ${active ? 'bg-green-50 text-green-700 border-green-200' : 'bg-(--surface-raised) text-(--text-secondary) border-(--border)'}`}
                      >
                        {active ? (
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        >
                          {isExpanded ? (
                            ...
                            <>
                              <ChevronDown className="w-3.5 h-3.5" /> Companies
                            </>
                          )}
                        </button>
                        <Link
                        ...
                        tenant.tenant_code === 'system' ? (
                          <span
                            className="text-xs font-medium flex items-center gap-1 text-(--text-muted) cursor-not-allowed opacity-50"
                            title="System plan cannot be changed"
                          >
                      ...
                      >
                        ...
                        {loadingCos ? (
                          ...
                          <p className="text-xs" style={S.muted}>
                            No companies registered under this tenant yet.
                          </p>
                        ) : (
                          <div className="flex flex-wrap gap-2">
                            {expandedCompanies.map((co: any) => (
                              <span
                                key={co.company_id}
                                className="text-xs border rounded-[var(--radius-sm)] px-3 py-1.5 font-medium shadow-sm flex items-center gap-1.5"
                              ...
                              >
                                ...
                                {co.company_name}
                                <span
                                  className={`text-xs font-bold ${co.onboarding_status === 'COMPLETED' ? 'text-green-600' : 'text-amber-500'}`}
                                >
        ...
        <form onSubmit={handleCreateTenant} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full border rounded-[var(--radius-sm)] px-3 py-2.5 text-sm focus:outline-none"
                  style={S.input}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCreateForm((f) => ({ ...f, plan_id: e.target.value }))
                  }
                  className="w-full border rounded-[var(--radius-sm)] px-3 py-2.5 text-sm focus:outline-none"
                  style={S.input}
                >
          ...
          <div className="space-y-4 pt-4 border-t" style={S.border}>
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-(--accent)">
                  3. Permitted Business Sectors (NOB & LOB)
                </h3>
                <p className="text-xs mt-0.5" style={S.muted}>
                  Select which Nature of Business (NOB) & Line of Business (LOB)
                  options this tenant is licensed to use.
                </p>
              </div>
              <button
                type="button"
                onClick={() => {
                  ...
                  const allLobIds = Object.values(lobsByNob)
                    .flat()
                    .map((l) => l.lob_id);
              ...
              >
                {selectedNobIds.length === nobsList.length
                  ? 'Deselect All'
                  : 'Select All'}
              </button>
            </div>
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
              {nobsList.length === 0 ? (
                <p className="text-xs italic" style={S.muted}>
                  No NOB sectors configured in master catalog.
                </p>
              ) : (
                      ...
                      {isNobChecked && childLobs.length > 0 && (
                        ...
                        >
                          {childLobs.map((lob: any) => {
                            const isLobChecked = selectedLobIds.includes(
                              lob.lob_id,
                            );
                            const toggleLob = () => {
                              if (isLobChecked) {
          ...
          >
            <button
              ...
              className="min-h-10 rounded-[var(--radius-sm)] border border-(--border) bg-(--surface) px-5 text-sm font-semibold text-(--text-secondary) hover:bg-(--surface-raised) disabled:opacity-50"
            >
              Cancel
            </button>
            <button

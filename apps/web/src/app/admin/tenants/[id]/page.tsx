...
function InfoRow({
  ...
  return (
    <div className="px-5 py-3.5 border-b" style={S.border}>
      ...
      <div className="text-sm font-semibold truncate" style={S.primary}>
        {value ?? '—'}
      </div>
    </div>
  );
...
function StatCard({
  ...
  return (
    <div className="rounded-[var(--radius-sm)] p-5 border" style={S.surface}>
      <div className="flex items-center justify-between mb-2">
        ...
        <Icon
          className="w-4 h-4"
          style={accent ? { color: 'var(--accent)' } : S.muted}
        />
      </div>
      <div className="text-2xl font-black" style={S.primary}>
        {value}
      </div>
      {sub && (
        <div className="text-xs mt-1 truncate" style={S.sub}>
          {sub}
        </div>
      )}
    </div>
...
function Badge({
  ...
  color?: 'green' | 'amber' | 'blue' | 'purple' | 'default';
}) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs font-semibold border px-2.5 py-0.5 rounded-full ${cls[color]}`}
    >
      {children}
    </span>
  );
...
function MiniCard({
  ...
  return (
    <div className="rounded-[var(--radius-sm)] border p-3" style={S.raised}>
      ...
      <div className="text-xs font-semibold truncate" style={S.primary}>
        {value ?? '—'}
      </div>
    </div>
  );
...
export default function TenantDetailPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setNobs(nobsList || []);
      setEditNobIds(
        Array.isArray(tenantData?.allowed_nob_ids)
          ? tenantData.allowed_nob_ids
          : nobsList.map((n: any) => n.nob_id),
      );
      setEditLobIds(
        Array.isArray(tenantData?.allowed_lob_ids)
          ? tenantData.allowed_lob_ids
          : [],
      );
      ...
      if (cos.length > 0) {
        ...
        const results = await Promise.allSettled(
          cos.map((co: any) =>
            api.get(`/setup/wizard/company-details/${co.company_id}`, {
  ...
  const handleUpgrade = async (e: React.FormEvent) => {
    ...
    setSuccess('');
    try {
      await api.post(`/tenant/${tenantId}/change-plan`, {
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 xl:p-8">
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl font-black" style={S.primary}>
              {tenant.tenant_name}
            </h1>
            <p
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
          ...
          <InfoRow
            ...
            value={
              tenant.api_rate_limit ? `${tenant.api_rate_limit} req/hr` : '—'
            }
          />
          ...
          <InfoRow
            label="DB Host"
            value={`${tenant.db_host || 'localhost'}:${tenant.db_port || 3306}`}
          />
      ...
      >
        ...
        >
          ...
          <button
            onClick={() => {
              setEditNobIds(
                Array.isArray(tenant.allowed_nob_ids) &&
                  tenant.allowed_nob_ids.length > 0
                  ? tenant.allowed_nob_ids
                  : nobs.map((n) => n.nob_id),
              );
              setEditLobIds(
                Array.isArray(tenant.allowed_lob_ids) &&
                  tenant.allowed_lob_ids.length > 0
        ...
        <div className="p-6">
          {!tenant.allowed_nob_ids || tenant.allowed_nob_ids.length === 0 ? (
            <p className="text-xs font-semibold text-green-600 bg-green-50 border border-green-200 rounded-[var(--radius-sm)] p-3">
              Full Unrestricted Sector Access — All Nature of Business (NOB) and
              Line of Business (LOB) sectors are permitted for this tenant.
            </p>
          ) : (
              ...
              <div className="flex flex-wrap gap-2">
                {nobs
                  ...
                  .map((n) => (
                    <div
                      key={n.nob_id}
                      className="rounded-[var(--radius-sm)] border px-3 py-2 text-xs font-bold flex flex-col gap-1"
                      style={{
                        ...
                        {lobs
                          .filter(
                            (l) =>
                              ...
                              (!tenant.allowed_lob_ids ||
                                tenant.allowed_lob_ids.length === 0 ||
                                tenant.allowed_lob_ids.includes(l.lob_id)),
                          )
                          .map((l) => (
                            <span
                              key={l.lob_id}
                              className="text-xs font-normal px-2 py-0.5 rounded bg-black/20 text-white"
                            >
                              {l.lob_name}
                            </span>
                          ))}
      ...
      >
        <div className="flex items-start gap-3">
          <div className="rounded-[var(--radius-sm)] bg-amber-50 p-2 text-amber-600">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm" style={S.sub}>
              {isSystemTenant
                ? 'The platform tenant uses a fixed plan.'
                : `Currently on ${planLabel}.`}
            </p>
          </div>
        </div>
        {!isSystemTenant && (
          <button
            ...
            className="min-h-10 rounded-[var(--radius-sm)] bg-(--accent) px-4 text-sm font-semibold text-white hover:bg-(--accent-hover)"
          >
            Change plan
          </button>
        )}
      ...
      <Dialog
        open={showPlanDialog}
        onClose={() => !upgrading && setShowPlanDialog(false)}
        title="Change subscription plan"
        description={`Choose a new plan for ${tenant.tenant_name}.`}
        maxWidth="sm"
      >
        <form onSubmit={handleUpgrade} className="space-y-5">
          <div>
            ...
            <select
              ...
              className="min-h-11 w-full rounded-[var(--radius-sm)] border px-4 text-sm"
              style={S.input}
            >
              <option value="">— Select a plan —</option>
              {plans.map((p) => (
                <option key={p.plan_id} value={p.plan_id}>
                  {p.plan_name} — {p.billing_cycle} · ${p.price}
                </option>
              ))}
            </select>
          </div>
          ...
          >
            <button
              ...
              className="min-h-10 rounded-[var(--radius-sm)] border border-(--border) bg-(--surface) px-4 text-sm font-semibold text-(--text-secondary) hover:bg-(--surface-raised) disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={
                !selectedPlan || upgrading || selectedPlan === tenant?.plan_id
              }
              className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-(--accent) px-5 text-sm font-semibold text-white hover:bg-(--accent-hover) disabled:opacity-50"
            >
              {upgrading ? (
            ...
            <tbody>
              {companies.length === 0 ? (
                ...
                companies.map((co, idx) => {
                  ...
                  const regNo =
                    d?.company?.registration_no || co.company_reg_no || '—';
                  const country = d?.company?.country_id || co.country || '—';
                  const currency =
                    d?.company?.base_currency_id || co.currency_id || '—';
                  const modules = d?.modules || [];

                  const nobCodes = modules.filter(
                    (m: string) => !m.includes('_'),
                  );
                  const lobCodes = modules.filter((m: string) =>
                    m.includes('_'),
                  );
                  ...
                  const currencyObj = currencies.find(
                    (c) =>
                      c.currency_id === currency || c.iso_code === currency,
                  );
                  const currencyName = currencyObj
                    ? `${currencyObj.currency_name} (${currencyObj.iso_code})`
                    : currency;
                      ...
                      {isExpanded && (
                        <tr key={`${co.company_id}-expanded`}>
                          ...
                          >
                            {isLoading ? (
                              <div
                                className="flex items-center gap-2 px-8 py-6 text-sm"
                                style={S.muted}
                              >
                              ...
                              <div
                                className="px-6 py-5 space-y-6 border-l-4"
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
                                    <MiniCard
                                      label="Legal Name"
                                      value={d.company?.company_name}
                                    />
                                    <MiniCard
                                      label="Display Name"
                                      value={
                                        d.company?.company_display_name ||
                                        d.company?.company_name
                                      }
                                    />
                                    <MiniCard
                                      label="Type"
                                      value={d.company?.company_type}
                                    />
                                    <MiniCard
                                      label="Industry"
                                      value={d.company?.industry_type}
                                    />
                                    <MiniCard
                                      label="Tax ID"
                                      value={d.company?.tax_id}
                                    />
                                    <MiniCard
                                      label="Country"
                                      value={countryName}
                                ...
                                {d.contact && (
                                  <div>
                                    <p
                                      ...
                                      style={S.muted}
                                    >
                                      <Phone className="w-3.5 h-3.5" /> Primary
                                      Contact
                                    </p>
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                      <MiniCard
                                        label="Name"
                                        value={d.contact.full_name}
                                      />
                                      <MiniCard
                                        label="Designation"
                                        value={d.contact.designation}
                                      />
                                      <MiniCard
                                        label="Email"
                                ...
                                {d.fiscal && (
                                  <div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                                      <MiniCard
                                        label="Fiscal Year"
                                        value={d.fiscal.current_fiscal_year}
                                      />
                                      <MiniCard
                                        label="Start Month"
                                        value={d.fiscal.fiscal_start_month}
                                      />
                                      <MiniCard
                                        label="Period Type"
                                        value={d.fiscal.period_type}
                                      />
                                      <MiniCard
                                        label="Standard"
                                        value={d.fiscal.accounting_standard}
                                      />
                                      <MiniCard
                                        label="Inventory Val."
                                        value={d.fiscal.inventory_valuation}
                                      />
                                      <MiniCard
                                        label="FY Format"
                                        value={d.fiscal.fiscal_year_format}
                                      />
                                    </div>
                                  </div>
                                )}
                                ...
                                {modules.length > 0 && (
                                  <div>
                                    <p
                                      ...
                                      style={S.muted}
                                    >
                                      <Layers className="w-3.5 h-3.5" /> Enabled
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {modules.map((m: string) => {
                                        ...
                                        const displayName = isLob
                                          ? lobs.find((l) => l.lob_code === m)
                                              ?.lob_name || m
                                          : nobs.find((n) => n.nob_code === m)
                                              ?.nob_name || m;
                                        return (
                                          <span
                                            key={m}
                                            className={`text-xs font-bold px-2.5 py-1 rounded-full border ${isLob ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-purple-200 bg-purple-50 text-purple-700'}`}
                                          >
                                            {displayName}
                                          </span>
                                        );
                                ...
                                {/* Associated Members */}
                                {(() => {
                                  const coUsers = users.filter(
                                    (u) => u.company_id === co.company_id,
                                  );
                                  return coUsers.length > 0 ? (
                                    <div>
                                      ...
                                      >
                                        ...
                                        Associated Members ({coUsers.length})
                                      </p>
                                      <div
                                        className="rounded-[var(--radius-sm)] border overflow-hidden"
                                        style={S.surface}
                              ...
                              <p className="px-8 py-5 text-xs" style={S.muted}>
                                No detailed setup found for this company.
                              </p>
                            )}
                          </td>
                        </tr>
                ...
                users.map((u, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 text-xs" style={S.sub}>
                      {u.company_id ? (
                        <div>
                          <div className="font-semibold" style={S.primary}>
                            {companyMap[u.company_id] || 'Unknown'}
                          </div>
                          <div
                            className="font-mono text-xs mt-0.5"
                            style={S.muted}
                          >
                            {u.company_id.substring(0, 12)}…
                          </div>
                        </div>
      ...
      {showSectorModal && (
        ...
        >
          <div
            className="rounded-[var(--radius-md)] border shadow-[var(--shadow-md)] w-full max-w-2xl my-auto"
            style={S.surface}
          >
            <div
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold" style={S.primary}>
                  Configure Permitted Business Sectors
                </h2>
              </div>
              <button
            ...
            <div className="px-6 py-5 space-y-4">
              <p className="text-xs" style={S.sub}>
                Select which Nature of Business (NOB) and Line of Business (LOB)
                sectors this tenant is permitted to configure during company
                onboarding.
              </p>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                {nobs.map((nob: any) => {
                  ...
                  const toggleNob = () => {
                    if (isNobChecked) {
                      setEditNobIds((prev) =>
                        prev.filter((id) => id !== nob.nob_id),
                      );
                      const childIds = new Set(childLobs.map((l) => l.lob_id));
                      setEditLobIds((prev) =>
                        prev.filter((id) => !childIds.has(id)),
                      );
                    } else {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        backgroundColor: isNobChecked
                          ? 'var(--accent-muted)'
                          : 'var(--surface-raised)',
                        borderColor: isNobChecked
                          ? 'var(--accent)'
                          : 'var(--border)',
                      }}
                    >
                      ...
                      {isNobChecked && childLobs.length > 0 && (
                        <div
                          className="mt-2.5 ml-6 pt-2 border-t flex flex-wrap gap-2"
                          style={{ borderColor: 'var(--border)' }}
                        >
                          {childLobs.map((lob: any) => {
                            const isLobChecked = editLobIds.includes(
                              lob.lob_id,
                            );
                            const toggleLob = () => {
                              if (isLobChecked) {
                                setEditLobIds((prev) =>
                                  prev.filter((id) => id !== lob.lob_id),
                                );
                              } else {
                            ...
                            return (
                              <label
                                key={lob.lob_id}
                                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-medium border cursor-pointer select-none"
                                style={{
                                  backgroundColor: isLobChecked
                                    ? 'var(--surface)'
                                    : 'transparent',
                                  borderColor: isLobChecked
                                    ? 'var(--accent)'
                                    : 'var(--border)',
                                  color: isLobChecked
                                    ? 'var(--accent)'
              ...
              <div className="flex gap-3 pt-3 border-t" style={S.border}>
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setSuccess(
                        'Tenant permitted business sectors updated successfully.',
                      );
                      setShowSectorModal(false);
                      await loadAll();
                    } catch (err: any) {
                      setError(
                        err?.message || 'Failed to update permitted sectors.',
                      );
                    } finally {
                      setSavingSectors(false);
                    }
                  }}
                  className="flex items-center gap-2 px-5 py-2.5 text-white text-sm font-semibold rounded-[var(--radius-sm)] disabled:opacity-50 cursor-pointer"
                  style={{ backgroundColor: 'var(--accent)' }}
                >
                  {savingSectors ? (
                    <RefreshCw className="w-4 h-4 animate-spin" />
                ...
                <button
                  type="button"
                  onClick={() => setShowSectorModal(false)}
                  className="px-5 py-2.5 text-sm font-medium rounded-[var(--radius-sm)] border cursor-pointer"

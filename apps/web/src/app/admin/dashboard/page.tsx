...
export default function AdminDashboardPage() {
  ...
  const actionBadge = (action: string) => {
    ...
    return (
      <span
        className="text-xs font-bold uppercase font-mono px-2 py-0.5 rounded border"
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 xl:p-8">
      ...
      <div>
        <div>
          <h1 className="text-2xl font-black tracking-tight" style={S.primary}>
            {t('controlTowerDashboard')}
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Total Tenants */}
        <div
          className="rounded-[var(--radius-md)] border p-5 shadow-sm transition-all hover:shadow-md"
          style={S.surface}
        >
          ...
          <div className="text-3xl font-black" style={S.primary}>
            {totalTenants}
          </div>
          <div className="mt-2 text-xs flex items-center gap-2" style={S.sub}>
            ...
            <span className="font-semibold" style={S.muted}>
              {inactiveTenants} {t('inactive')}
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div className="text-base font-extrabold truncate" style={S.primary}>
            localhost:3306
          </div>
          <div className="mt-2 text-xs flex items-center gap-1.5" style={S.sub}>
            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse inline-block" />
            <span className="text-green-600 font-bold">
              {t('sqlConnectionsHealthy')}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Plan Breakdown Card */}
        <div
          className="lg:col-span-1 rounded-[var(--radius-md)] border p-6 space-y-6 shadow-sm"
          style={S.surface}
        >
          <div>
            <h3
              ...
              style={S.sub}
            >
              {t('tenantSubscriptionShare')}
            </h3>
            <p className="text-xs" style={S.muted}>
              {t('planDistributionAcrossActive')}
            </p>
          </div>
              ...
              return (
                <div key={p.plan_id} className="space-y-1.5">
                  <div className="flex justify-between text-xs font-semibold">
                    ...
                    <span style={S.sub}>
                      {count} tenant{count !== 1 ? 's' : ''} ({pct.toFixed(0)}%)
                    </span>
                  </div>
                  <div
        ...
        <div
          className="lg:col-span-2 rounded-[var(--radius-md)] border overflow-hidden shadow-sm"
          style={S.surface}
        >
          <div
          ...
          >
            <div>
              <h3
                ...
                style={S.sub}
              >
                {t('recentTenantRegistrations')}
              </h3>
              <p className="text-xs" style={S.muted}>
                {t('newlyOnboardedEntities')}
              </p>
            </div>
            <Link
              href="/admin/tenants"
          ...
          <div className="divide-y" style={S.border}>
            {tenants.length === 0 ? (
              <div className="px-6 py-10 text-center text-sm" style={S.muted}>
                {t('noTenantsRegistered')}
              </div>
            ) : (
              tenants
                .slice(-4)
                ...
                .map((tItem) => (
                  <div
                    key={tItem.tenant_id}
                    className="px-6 py-4 flex items-center justify-between gap-4 flex-wrap hover:bg-(--surface-raised) transition-colors"
                  >
                    <div>
                      <div className="text-sm font-bold" style={S.primary}>
                        {tItem.tenant_name}
                      </div>
                      <div className="text-xs font-mono" style={S.muted}>
                        {tItem.tenant_code} · {tItem.billing_email}
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-xs font-bold ${tItem.is_active !== false ? 'text-green-600' : ''}`}
                        style={tItem.is_active !== false ? undefined : S.muted}
                      >
                        {tItem.is_active !== false
                          ? t('active')
                          : t('inactive')}
                      </span>
                    </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-3">
          <h3
            ...
            style={S.muted}
          >
            {t('managementControls')}
          </h3>

          <div className="grid grid-cols-1 gap-3">
            {[
              {
                label: t('addNewTenant'),
                icon: Plus,
                color: '#10B981',
                href: '/admin/tenants',
                desc: t('provisionNewDbAdmin'),
              },
              {
                label: t('configurePricingPlans'),
                icon: Layers,
                color: '#F59E0B',
                href: '/admin/plans',
                desc: t('manageQuotasRates'),
              },
              {
                label: t('masterRegistries'),
                icon: Database,
                color: '#3B82F6',
                href: '/admin/masters',
                desc: t('seedGlobalStaticCatalogs'),
              },
              {
                label: t('complianceAuditLogs'),
                icon: History,
                color: '#8B5CF6',
                href: '/admin/audit',
                desc: t('platformSecurityActivity'),
              },
            ].map((action) => (
              <Link
                key={action.label}
                href={action.href}
                className="rounded-[var(--radius-md)] border p-4 flex items-start gap-3.5 shadow-sm transition-all hover:translate-x-1 hover:shadow-md"
                style={S.surface}
              >
                <div
            ...
            {auditLogs.length === 0 ? (
              ...
              auditLogs.map((log, index) => (
                <div
                  ...
                  className="px-6 py-3.5 flex items-center justify-between gap-4 flex-wrap hover:bg-(--surface-raised) transition-colors"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      {actionBadge(log.action)}
                      <span className="text-xs font-bold" style={S.primary}>
                        {log.entity_name}
                      </span>
                    </div>

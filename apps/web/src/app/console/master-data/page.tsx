...
import {
  ...
  MASTER_DATA_GROUPS,
  getConfig,
} from '@/modules/master-data/configs';
        ...
        <div className="flex items-center gap-3 rounded-[var(--radius-lg)] border border-amber-200 bg-amber-50 p-5 text-amber-800">
          ...
          <div>
            <p className="text-sm font-semibold">
              Master Data is managed by Company Admins
            </p>
            <p className="mt-1 text-xs">
              Your account ({user.userType.replace(/_/g, ' ').toLowerCase()})
              doesn&apos;t have access to this section. Contact your company
              administrator if you need a change made here.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-7">
      <div className="mb-5">
        ...
        <p className="mt-0.5 text-sm" style={S.sub}>
          Operational reference data for your company — farms, inventory,
          livestock, partners and finance dimensions.
        </p>
      </div>
          ...
          >
            {MASTER_DATA_GROUPS.map((group) => (
              <div key={group} className="mb-1 last:mb-0">
                <p
                  className="px-3 pb-1 pt-3 text-xs font-bold uppercase tracking-widest"
                  style={S.muted}
                >
                  {group}
                </p>
                <ul className="flex flex-col gap-0.5">
                  {MASTER_DATA_CONFIGS.filter((c) => c.group === group).map(
                    (c) => {

...
const inputCls =
  'w-full border rounded-[var(--radius-sm)] px-3 py-2.5 text-sm focus:ring-1';
const inputStyle = {
...
export default function NotificationsPage() {
  ...
  const loadData = async (storedUser: NavUser, tid: string) => {
    ...
    try {
      ...
      const myCompany =
        companiesList.find((c: any) => c.company_id === myId) ||
        companiesList[0];
      ...
      if (myCompany?.company_id) {
        const [configs, logsList] = await Promise.all([
          api
            .get(`/notification/company/${myCompany.company_id}`)
            .catch(() => []),
        ...
        if (emailCfg)
          setEmailForm({
            ...
            smtp_password_enc: emailCfg.smtp_password_enc || '',
            from_email: emailCfg.from_email || '',
            from_name: emailCfg.from_name || 'NAVFarm Alerts',
          });
        if (webhookCfg)
          setWebhookForm({
            webhook_url: webhookCfg.webhook_url || '',
            webhook_secret_enc: webhookCfg.webhook_secret_enc || '',
          });
      }
    } catch (e: any) {
  ...
  const handleSaveEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existing) {
        ...
        const created = await api.post('/notification', {
  ...
  const handleTestSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const configs = await api
        .get(`/notification/company/${activeCompany.company_id}`)
        .catch(() => []);
      const emailCfg = configs?.find((c: any) => c.channel === 'EMAIL');
      if (!emailCfg) {
        throw new Error(
          'Please configure and save SMTP details first before sending a test notification.',
        );
      }

      await api.post('/notification/test', {
        configId: emailCfg.notif_id,
      ...
      // Reload logs
      const updatedLogs = await api
        .get(`/notification/logs/${activeCompany.company_id}`)
        .catch(() => []);
  ...
  const tabCls = (tab: string) =>
    `px-5 py-2.5 text-sm font-semibold border-b-2 transition-colors cursor-pointer flex items-center gap-2 ${
      activeChannel === tab ? 'border-[var(--accent)]' : 'border-transparent'
    }`;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6 xl:p-8">
      ...
      {/* Channel Tabs */}
      <div
        className="rounded-[var(--radius-sm)] border shadow-sm"
        style={{
      ...
      {/* Test Send */}
      <div
        className="flex flex-col gap-4 rounded-[var(--radius-md)] border bg-(--surface) p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between"
        style={{ borderColor: 'var(--border)' }}
      >
        <div className="flex items-start gap-3">
          <div className="rounded-[var(--radius-sm)] bg-blue-50 p-2 text-blue-700">
        ...
        <button
          ...
          className="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-[var(--radius-sm)] bg-(--accent) px-4 text-sm font-semibold text-white transition hover:bg-(--accent-hover)"
        >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4">
                        ...
                        {log.error_message && (
                          <div className="text-xs text-rose-500 mt-1 font-mono">
                            {log.error_message}
                          </div>
                        )}
                      </td>
                      <td className="p-4 text-center">
                        <span
                          className={`text-xs font-bold border px-2 py-0.5 rounded inline-flex items-center gap-1 ${
                            isSuccess

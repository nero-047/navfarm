...
function ThemeIconButton() {
  ...
  return (
    <button
      ...
      className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-raised)] hover:text-[var(--color-navy)]"
    >
...
export default function ConsoleLayout({
  ...
  const [currentActiveCompanyId, setCurrentActiveCompanyId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (tenantId) {
      api
        .get(`/tenant/${tenantId}`)
        .then((data: any) => setTenantPlanInfo(data))
        .catch(() => setTenantPlanInfo(null));
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          <span
            className="ml-2 hidden text-xs font-semibold uppercase tracking-widest px-2 py-0.5 rounded sm:inline-flex"
            style={{
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              ...
              aria-label="Sign out"
              className="text-sm flex h-10 items-center gap-1.5 rounded-[var(--radius-md)] px-2 sm:px-3"
              style={{ color: 'var(--text-secondary)' }}
            >
  ...
  const navItems: ConsoleSidebarItem[] = [
    {
      ...
      show:
        user.userType === 'TENANT_ADMIN' || user.userType === 'COMPANY_ADMIN',
    },
    {
      label: 'Companies',
      href: '/console/companies',
      icon: Building2,
      show: hasPermission(user, 'COMPANY', 'SETTINGS', 'can_view'),
    },
    {
      label: 'Master Data',
      href: '/console/master-data',
      icon: Database,
      show: hasPermission(user, 'MASTER_DATA', 'UOM', 'can_view'),
    },
    {
      label: 'Inventory',
      href: '/console/inventory',
      icon: Boxes,
      show: hasPermission(user, 'INVENTORY', 'GOODS_RECEIPT', 'can_view'),
    },
    {
      label: 'Finance',
      href: '/console/finance',
      icon: Landmark,
      show: hasPermission(user, 'FINANCE', 'JOURNAL', 'can_view'),
    },
    {
      label: 'Production',
      href: '/console/production',
      icon: Sprout,
      show: hasPermission(user, 'PRODUCTION', 'BATCH', 'can_view'),
    },
    {
      label: 'Team Management',
      href: '/console/users',
      icon: Users,
      show: hasPermission(user, 'RBAC', 'USER', 'can_view'),
    },
    {
      label: 'Role Permissions',
      href: '/console/roles',
      icon: ShieldAlert,
      show: hasPermission(user, 'RBAC', 'ROLE', 'can_view'),
    },
    {
      label: 'Audit Ledger',
      href: '/console/audit',
      icon: History,
      show: hasPermission(user, 'AUDIT', 'LOGS', 'can_view'),
    },
    {
      label: 'Notifications',
      href: '/console/notifications',
      icon: Bell,
      show: hasPermission(user, 'NOTIFICATION', 'SETTINGS', 'can_view'),
    },
  ].filter((i) => i.show);
  ...
  const SidebarContent = () => {
    const planName = (
      tenantPlanInfo?.plan_id?.replace('PLAN_', '') || 'STANDARD'
    ).toUpperCase();
    return (
      <div className="flex h-full flex-col">
        <div className="border-b border-white/[0.08] px-5 pb-4 pt-5">
          <Link href="/console/dashboard" className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-sm)] border border-white/15 text-sm font-black text-white">
              NF
            </span>
            <span>
          ...
          <div className="mt-5 rounded-[var(--radius-md)] border border-white/10 bg-white/[0.06] p-3">
        ...
        <nav className="flex-1 overflow-y-auto p-3">
          <p className="px-3 pb-2 pt-2 text-xs font-medium text-white/40">
            Organization
          </p>
              ...
              return (
                <li key={item.href}>
                  <Link
                    ...
                    className={`group flex min-h-11 items-center gap-3 rounded-[var(--radius-sm)] px-3 text-[14px] transition-colors ${isActive ? 'bg-white/10 font-semibold text-white' : 'text-white/70 hover:bg-white/[0.06] hover:text-white'}`}
                  >
                    <item.icon
                      size={18}
                      className={isActive ? 'text-[#ef806f]' : ''}
                    />
                    {item.label}
                  </Link>
                </li>
        ...
        <div className="border-t border-white/[0.08] p-4">
          <div className="flex items-center gap-3 px-1 pb-3">
            <span className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-white/10 text-xs font-bold text-white">
              {initials}
              <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-(--sidebar-bg) bg-emerald-400" />
            </span>
          ...
          <button
            onClick={handleLogout}
            className="flex w-full items-center justify-center gap-2 rounded-[var(--radius-md)] border border-white/10 px-3 py-2.5 text-xs font-medium text-white/50 transition hover:border-red-400/25 hover:bg-red-400/10 hover:text-red-300"
          >
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] lg:flex">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-white/10 bg-(--sidebar-bg) text-white lg:flex">
        <SidebarContent />
      </aside>
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <aside className="relative h-full w-[min(320px,88vw)] bg-(--sidebar-bg)">
            <button
              onClick={() => setSidebarOpen(false)}
              aria-label="Close navigation"
              className="absolute right-3 top-3 z-10 flex h-10 w-10 items-center justify-center rounded-[var(--radius-md)] text-white/60 hover:bg-white/10 hover:text-white"
            >
              <X size={19} />
            </button>
            <SidebarContent />
          ...
          >
            ...
            {user?.companies &&
              ...
              (() => {
                const active =
                  user.companies.find(
                    (c) => c.company_id === currentActiveCompanyId,
                  ) || user.companies[0];
                return (
                      ...
                      <>
                        ...
                        >
                          ...
                          {user.companies.map((c: CompanyRef) => {
                            const isCurrent =
                              c.company_id === currentActiveCompanyId;
                            return (
                              <button
                                key={c.company_id}
                                onClick={() => {

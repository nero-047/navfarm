    ...
    <AdminShell title="Organization" eyebrow={tenant?.tenant_name || 'Tenant workspace'}>
      ...
      <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map(([label, value, Icon, detail]) => (
          <div key={label} className="rounded-2xl border border-(--border) bg-(--surface) p-5"><div className="flex justify-between"><div><p className="text-xs text-(--text-secondary)">{label}</p><p className="mt-2 text-2xl font-semibold text-(--text-primary)">{value}</p></div><Icon size={19} className="text-(--accent)" /></div><p className="mt-3 text-xs text-(--text-secondary)">{detail}</p></div>
        ))}
      </div>

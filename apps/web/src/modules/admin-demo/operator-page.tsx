...
export function OperatorPage() {
  ...
  return <AdminShell title="Platform operator" eyebrow="Cross-tenant control plane">
    ...
    <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
      ['Active tenants', '6', Building2, '1 trial'], ['Companies', '8', Boxes, 'Across 6 NOBs'], ['Active users', '42', Users, 'Plan capacity 75'], ['Languages', '16', Languages, 'English fallback'],
    ].map(([label, value, Icon, detail]) => <div key={String(label)} className="rounded-2xl border border-[#e7e7e7] bg-white p-5"><div className="flex justify-between"><div><p className="text-xs text-[#707070]">{String(label)}</p><p className="mt-2 text-2xl font-semibold">{String(value)}</p></div><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-[#1c4aa9]"><Icon size={18}/></div></div><p className="mt-3 text-xs text-[#707070]">{String(detail)}</p></div>)}</div>

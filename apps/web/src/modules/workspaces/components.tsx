...
export function WorkspaceList() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      {items.map((workspace) => <button key={workspace.workspaceId} onClick={() => void open(workspace)} className="min-h-36 rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:border-blue-400 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">
      ...
      {!items.length ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-sm text-slate-600"><h2 className="font-bold text-slate-900">No workspaces configured</h2><p className="mt-2">{canManage ? 'Create the first business area, then assign operational users.' : 'Ask a tenant administrator to configure and assign a workspace.'}</p></div> : null}
    </div>
  </div>;
}
...
export function WorkspaceDetail({ workspaceSlug }: { workspaceSlug: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const list = await workspaceClient.list(companyMembership.tenantId, companyMembership.companyId);
      const match = list.find((item) => item.workspaceSlug === workspaceSlug);
      if (!match) throw new ApiError('Workspace not found.', 404, 'NOT_FOUND');
      const [detail, membershipRows] = await Promise.all([workspaceClient.get(companyMembership.companyId, match.workspaceId), workspaceClient.members(companyMembership.companyId, match.workspaceId)]);
      setWorkspace(detail); setMembers(membershipRows);
    } catch (cause) { setError(message(cause)); } finally { setLoading(false); }
  }, [companyMembership, workspaceSlug]);
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6"><h2 className="font-bold">Details and modules</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">Workspace name<input disabled={!canManage} value={workspace.workspaceName} onChange={(event) => setWorkspace((current) => current ? { ...current, workspaceName: event.target.value } : current)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-300 px-3 font-normal disabled:bg-slate-50" /></label><label className="text-sm font-semibold">Status<select disabled={!canManage} value={workspace.status} onChange={(event) => setWorkspace((current) => current ? { ...current, status: event.target.value as Workspace['status'] } : current)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-300 px-3 font-normal disabled:bg-slate-50"><option value="DRAFT">Draft</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select></label></div><div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{moduleOptions.map((module) => <label key={module} className="flex min-h-11 items-center gap-3 rounded-xl border border-slate-200 px-3 text-sm"><input disabled={!canManage} type="checkbox" checked={workspace.enabledModules.includes(module)} onChange={() => toggleModule(module)} />{module}</label>)}</div>{canManage ? <button disabled={saving} onClick={() => void save()} className="mt-5 min-h-11 rounded-xl bg-[#0b1248] px-4 text-sm font-semibold text-white disabled:opacity-60">{saving ? 'Saving…' : 'Save workspace'}</button> : null}</section>

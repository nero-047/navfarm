...
ALTER TABLE `animal_register` ADD CONSTRAINT `animal_register_current_location_id_location_master_location_id_fk` FOREIGN KEY (`current_location_id`) REFERENCES `location_master`(`location_id`) ON DELETE restrict ON UPDATE no action;--> statement-breakpoint

...
export class GlMappingService {
  ...
  async create(dto: CreateGlMappingDto, tenantId: string, userPayload?: any) {
    ...
    const newMapping = {
      ...
      item_posting_group: dto.item_posting_group || null,
      valuation_method: dto.valuation_method ? dto.valuation_method.toUpperCase() : null,
  ...
  // FIX-012 (GAP-003): Seed GL Mappings for all V2 transaction types
  async seedV2DefaultMappings(companyId: string, tenantId: string, userPayload?: any) {
    const v2TransactionTypes = [
      {
        transaction_type: 'AQUA_HARVEST',
        debit_gl_account_id: '1300-HARVESTED-FISH', // Inventory Asset
        credit_gl_account_id: '1400-AQUA-WIP',     // Aquaculture WIP
      },
      {
        transaction_type: 'FEED_MO_OUTPUT',
        debit_gl_account_id: '1310-FINISHED-FEED',  // Finished Feed Inventory
        credit_gl_account_id: '1410-FEED-WIP',       // Feed Manufacturing WIP
      },
      {
        transaction_type: 'CROP_HARVEST',
        debit_gl_account_id: '1320-CROP-INVENTORY', // Crop Inventory Asset
        credit_gl_account_id: '1420-AGRI-WIP',       // Agri Production WIP
      },
      {
        transaction_type: 'LIVESTOCK_SALE',
        debit_gl_account_id: '1100-ACCOUNTS-REC',   // AR / Cash
        credit_gl_account_id: '4100-LIVESTOCK-REV',  // Biological Asset Revenue
      },
    ];

    const created: any[] = [];
    for (const item of v2TransactionTypes) {
      // Check if mapping already exists for this company & transaction type
      const [existing] = await this.db
        .select()
        .from(schema.glMappingMaster)
        .where(
          and(
            ...
            eq(schema.glMappingMaster.transaction_type, item.transaction_type),
      ...
      if (!existing) {
        ...
        const newRecord = {
          ...
          debit_gl_account_id: item.debit_gl_account_id,
          credit_gl_account_id: item.credit_gl_account_id,
          is_active: true,
          status: 'ACTIVE',
          created_by: userPayload?.userId || null,
        };
        await this.db.insert(schema.glMappingMaster).values(newRecord);
        created.push(newRecord);
      }
    }

    return { seededCount: created.length, created };
  }
}

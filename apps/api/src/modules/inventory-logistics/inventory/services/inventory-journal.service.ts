  ...
  async post(journalId: string, tenantId: string, userId?: string) {
    return this.db.transaction(async (trx) => {
      ...
      for (const line of journal.lines) {
        ...
        // FIX-023 (GAP-023): Automatic financial GL posting for Inventory Journal
        const lineTotal = Math.abs(line.qty * line.unit_cost);
        if (lineTotal > 0) {
          await this.postingEngineService.postAutomaticEntry({
            ...
            ref_doc_id: journalId,
            ref_doc_line_id: line.line_id,
            notes: `Inventory Journal GL posting for line ${line.line_id}`,
          }, tenantId, userId, trx);
        }
      }

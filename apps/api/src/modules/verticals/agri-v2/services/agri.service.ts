...
export class AgriV2Service {
  ...
  // ── PESTICIDE ──────────────────────────────────────────────────────────────
  async logPesticide(planId: string, dto: PesticideAppDto, tenantId: string, userId?: string) {
    const [plan] = await this.db.select().from(schema.agriCropPlan)
      .where(and(eq(schema.agriCropPlan.plan_id, planId), eq(schema.agriCropPlan.tenant_id, tenantId))).limit(1);
    if (!plan) throw new NotFoundException(`Crop plan not found.`);
    ...
    let giId: string | null = null;
    let inventoryNote = 'Pesticide logged without inventory deduction.';
    if (dto.pesticide_item_id && (dto as any).warehouse_id && (dto as any).location_id) {
      try {
        const gi = await this.goodsIssueService.create({
          company_id: plan.company_id,
          issue_type: 'CONSUMPTION',
          warehouse_id: (dto as any).warehouse_id,
          posting_date: dto.app_date,
          notes: `Pesticide application for crop plan ${planId}`,
          lines: [{
            item_id: dto.pesticide_item_id,
            location_id: (dto as any).location_id,
            qty: dto.qty_litre,
            uom_code: 'LITRE',
          }],
        }, tenantId, userId);
        if (gi?.issue_id) {
          await this.goodsIssueService.post(gi.issue_id, tenantId, userId);
          giId = gi.issue_id;
          inventoryNote = `Auto-posted Goods Issue ${gi.issue_no} for pesticide application.`;
        }
      } catch (err: any) {
        inventoryNote = `Pesticide logged; GI warning: ${err?.message || err}`;
      }
    }
